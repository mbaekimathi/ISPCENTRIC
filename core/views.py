...
from core.mikrotik_connect import (
    ...
    _reject_behind_provider_unbridge,
...
def _management_ready_for_uplink_apply(
    router: MikroTikRouter,
    live_ports: list[dict] | None,
) -> bool:
    """True when WireGuard API works or we already have a live RouterOS port list."""
    if _router_tunnel_verified(router, require_api=True):
        return True
    return bool(
        live_ports
        and any((p.get("name") or "").strip() for p in live_ports if isinstance(p, dict))
    )
...
def _invalidate_mikrotik_router_caches(org_pk: int, router_pk: int) -> None:
    cache.delete_many(
        [
            ...
            f"mikrotik_assigned_ports_live:{org_pk}:{router_pk}",
...
def build_mikrotik_detail_nav(
    ...
    *,
    ...
    recovery_url = reverse(
        "core:mikrotik_recovery_scripts", kwargs={"router_id": router.pk}
    )
    ...
    nav.extend(
        [
            ...
            {"key": "recovery_scripts", "label": "Winbox recovery", "href": recovery_url},
        ]
    )
    return nav
...
def mikrotik_assigned_ports(request, router_id: int):
    ...
    ctx = client_page_context(
        ...
        page_title=f"{router.name} — Live clients",
        page_subtitle="Who is online and which ISP link each customer uses.",
        router=router,
        ports_loading=not is_suspended,
        assigned_ports_live_url=(
            reverse("core:mikrotik_assigned_ports_live", args=[router.pk])
            if not is_suspended
            else ""
        ),
        assigned_ports_apply_url=(
            reverse("core:mikrotik_assigned_ports_apply", args=[router.pk])
            if not is_suspended
            else ""
        ),
        push_status_url=reverse("core:mikrotik_push_status", args=[router.pk]),
        ports_post_url=reverse("core:mikrotik_ports", args=[router.pk]),
        is_suspended=is_suspended,
...
def customer_supports_live_usage(customer) -> bool:
    ...
    if customer.service_type == Customer.ServiceType.PPPOE:
        # Router may be resolved later (assigned, discovered, or single-NAS fallback).
...
def resolve_client_usage_router(customer, org=None):
    ...
    if customer.service_type == Customer.ServiceType.HOTSPOT:
        ...
        if username and organization is not None:
            from core.mikrotik_connect import find_pppoe_router_for_username

            found = find_pppoe_router_for_username(organization, username)
            if found is not None and found.account_status != MikroTikRouter.AccountStatus.SUSPENDED:
                return found
    else:
        return None
...
def _multi_isp_uplink_modes() -> set[str]:
    """Modes that use two or more ISP links (always include failover)."""
    return {
        MikroTikRouter.UplinkMode.FAILOVER,
        MikroTikRouter.UplinkMode.BALANCE,
        MikroTikRouter.UplinkMode.SMART_BALANCE,
...
def _is_multi_isp_mode(mode: str) -> bool:
    return (mode or "").strip() in _multi_isp_uplink_modes()


# UI internet-setup goal shown in the ports dropdown (3 categories).
UPLINK_UI_GOAL_MULTI = "multi"


def _normalize_uplink_goal(goal: str) -> str:
    """
    Map UI / legacy goals to stored uplink_mode values.

    ``multi`` is the simplified UI choice for any multi-ISP setup and stores as
    smart_balance (failover + weighted share + slow-link avoidance).
    """
    goal = (goal or "").strip().lower()
    if goal == UPLINK_UI_GOAL_MULTI:
        return MikroTikRouter.UplinkMode.SMART_BALANCE
    valid = {
    ...
    return goal if goal in valid else ""


def _ui_uplink_goal(mode: str) -> str:
    """Collapse stored modes into the three operator-facing categories."""
    mode = (mode or MikroTikRouter.UplinkMode.SINGLE).strip()
    if mode == MikroTikRouter.UplinkMode.BOND:
        return MikroTikRouter.UplinkMode.BOND
    if _is_multi_isp_mode(mode):
        return UPLINK_UI_GOAL_MULTI
    return MikroTikRouter.UplinkMode.SINGLE


def _ui_uplink_goal_label(mode: str) -> str:
    goal = _ui_uplink_goal(mode)
    return {
        MikroTikRouter.UplinkMode.SINGLE: "One internet link",
        MikroTikRouter.UplinkMode.BOND: "Same ISP — combine cables",
        UPLINK_UI_GOAL_MULTI: "Multiple ISPs — failover + share",
    }.get(goal, "One internet link")


def _build_uplink_setup_status(
    *,
    uplink_mode: str,
    ...
    bond_router_applied: bool = False,
    wan_share: dict | None = None,
) -> dict:
    """
    Operator-facing readiness for multi-link setups.

    Tells the user when something is wrong vs when they can proceed / all is OK.
    """
    mode = (uplink_mode or MikroTikRouter.UplinkMode.SINGLE).strip()
    alerts = list(health_alerts or [])
    problems: list[str] = []
    # Operational health (failover active, primary down, etc.) belongs in the top
    # health banner only — not duplicated in setup status problems.
    _banner_only_health_codes = frozenset(
        {
            "failover_on_backup",
            "primary_wan_down",
            "primary_wan_no_isp",
            "multi_uplink_available",
            "balance_share_drift",
            "smart_balance_monitor_off",
        }
    )
    for alert in alerts:
        code = str((alert or {}).get("code") or "").strip()
        if code in _banner_only_health_codes:
            continue
        if (alert or {}).get("level") == "warn":
            msg = str((alert or {}).get("message") or "").strip()
            if msg:
                problems.append(msg)
    ...
    def _link_problems(names: list[str], *, need_internet: bool) -> list[str]:
        out: list[str] = []
        for name in names:
            row = by_name.get(name) or {}
            if row.get("disabled") or not row.get("running"):
                out.append(f"{name} has no link")
            elif need_internet and not _port_isp_ready_for_uplink(
                name,
                physical_ports,
                uplink_live=uplink_live,
                wan_share=wan_share,
            ):
                meta = _uplink_candidate_status(row)
                if meta.get("status") == "waiting_dhcp":
                    out.append(f"{name} is waiting for DHCP — ISP not verified yet")
                elif meta.get("status") == "waiting_pppoe":
                    out.append(f"{name} is waiting for PPPoE — ISP not verified yet")
                elif meta.get("status") == "link_up":
                    out.append(
                        f"{name} is link up only — wait for DHCP/PPPoE before applying"
                    )
                else:
                    out.append(f"{name} has no verified ISP yet")
        return out
    ...
    if mode == MikroTikRouter.UplinkMode.SINGLE:
        return {
            ...
            "can_proceed": True,
            "ok": True,
            "level": rec.get("level") or "ok",
            "title": rec.get("title") or "One internet link",
            "phase": rec.get("phase") or "ready",
            "message": rec.get("message")
            or "One internet link — change ports anytime; customers stay online.",
            "problems": [],
        }

    if mode == MikroTikRouter.UplinkMode.BOND:
        members = list(bond_member_ports)
        problems.extend(_link_problems(members, need_internet=False))
        applied = bool(bond_router_applied)
        ready = bool(bond_ready)
        if len(members) < 2:
            problems.append("Mark at least two ports as Bonded internet.")
            ready = False
        if problems:
            return {
                "applies": True,
                ...
                "message": "Bonding is not ready — fix the issues below, then apply.",
        ...
        if applied:
            return {
                ...
                "ok": True,
                "level": "ok",
                "title": "All OK — you can proceed",
                "phase": "applied",
                "message": "Bonding looks good — both cables are up. You can proceed.",
                "problems": [],
            }
        return {
            "applies": True,
            ...
            "title": (
                "Ready to apply"
                if ready
                else (rec.get("title") or "Waiting for bond cables")
            ),
            "phase": "ready_to_apply" if ready else "waiting",
            "message": (
                "All bond members look ready — click Apply bonding. "
                "The page stays usable while bonding runs in the background."
                if ready
                else (
                    rec.get("message")
                    or "Need two linked cables from the same ISP before bonding can apply."
                )
            ),
            "problems": [],
        }

    # Multi-ISP (failover / balance / smart_balance) — failover always applies.
    members = list(primary_wan_ports) + list(backup_wan_ports)
    if len(primary_wan_ports) != 1:
        problems.append("Set exactly one port as Internet (primary).")
    if len(backup_wan_ports) < 1:
        problems.append("Set at least one other ISP as Backup / Shared ISP.")
    if not balance_router_applied:
        problems.extend(_link_problems(members, need_internet=True))
    ...
    if mode == MikroTikRouter.UplinkMode.FAILOVER:
        applied = bool(
            uplink_live.get("ok")
            and (
                len(uplink_live.get("checked_routes") or []) >= 2
                or len(uplink_live.get("failover_clients") or []) >= 2
            )
        )
        ready = bool(dual_wan_ready)
    else:
        applied = bool(balance_router_applied) and (
            mode != MikroTikRouter.UplinkMode.SMART_BALANCE
            or smart_balance_applied is not False
        )
        ready = bool(balance_ready)

    unique_problems = list(dict.fromkeys(problems))
    if unique_problems:
        waiting = [
            str(p).strip()
            for p in (rec.get("waiting_ports") or [])
            if str(p).strip()
        ]
        message = (
            "Multiple ISP links need attention — fix these before proceeding. "
            "Failover covers every link once applied."
        )
        if waiting and any("verified ISP" in p or "link up" in p or "waiting" in p for p in unique_problems):
            message = (
                "Live links were found, but ISP is not confirmed on every Shared ISP yet. "
                "Wait for DHCP/PPPoE — we will notify you when it is ready to apply."
            )
        return {
            "applies": True,
            "ready": False,
    ...
    if applied and not unique_problems:
        return {
            "applies": True,
            "ready": True,
            "applied": True,
            ...
            "phase": "applied",
            "message": (
                "All ISP links look healthy — failover is active on every link. "
                "You can proceed."
            ),
            "problems": [],
        }
    ...
    return {
        ...
        "title": (
            "Ready to apply"
            if ready
            else (rec.get("title") or "Waiting for ISP links")
        ),
        "phase": "ready_to_apply" if ready else "waiting",
        "message": (
            "All ISP links look ready — Apply multi-ISP runs in the background "
            "(share traffic + failover on every link). Keep this page open."
            if ready
            else (
                rec.get("message")
                or "Waiting for two or more ISP links with internet before applying."
            )
        ),
        "problems": [],
    }
...
def _build_router_client_analysis(
    ...
    *,
    ...
    for index, port_name in enumerate(member_ports):
        ...
        status = "active"
        member_state = member_status.get(port_name)
        if port_name in slow_ports or member_state == "slow" or (
            isinstance(member_state, dict) and member_state.get("slow")
        ):
            status = "slow"
        elif isinstance(member_state, dict) and (
            member_state.get("disabled") or member_state.get("sidelined")
        ):
            status = "sidelined"
        download_bps = share_row.get("download_bps")
        upload_bps = share_row.get("upload_bps")
        isps.append(
            {
                "port": port_name,
                ...
                "download_label": share_row.get("download_label")
                or _usage_bps_label(download_bps)
                if download_bps is not None
                else "—",
                "upload_label": share_row.get("upload_label")
                or _usage_bps_label(upload_bps)
                ...
                "online_clients": 0,
                "connection_count": 0,
                "download_bps_clients": 0,
                "upload_bps_clients": 0,
                "bytes_total": 0,
            }
        )
    if not isps:
    ...
    lan_ports_map: dict[str, dict] = {}

    def _ensure_lan_port(port_name: str) -> dict | None:
        port_name = (port_name or "").strip()
        if not port_name:
            return None
        ...
        if row:
            return row
        role = (roles.get(port_name) or "").strip().lower()
        row = {
            "port": port_name,
            "label": port_name,
            "kind": "lan",
            "role": role or MikroTikRouter.PortRole.LAN,
            "status": "active",
            "client_count": 0,
            "online_clients": 0,
            "connection_count": 0,
    ...
    for name, role in roles.items():
        ...
        } or role_l.startswith("lan"):
            _ensure_lan_port(name)
    ...
    def _append_client(customer: Customer, *, ip: str, usage_row: dict | None) -> None:
        ...
        pin_row = client_pins.get(ip) if ip else {}
        isp_port = (usage_row.get("isp_port") or default_isp or "").strip()
        isp_pinned = bool((pin_row or {}).get("pinned"))
        if isp_pinned and (pin_row.get("isp_port") or "").strip():
            isp_port = (pin_row.get("isp_port") or "").strip()
        ...
        upload_bps = usage_row.get("upload_bps")
        if upload_bps is None:
            upload_bps = session_meta.get("upload_bps")
        bytes_in = int(usage_row.get("bytes_in") or session_meta.get("bytes_in") or 0)
        bytes_out = int(usage_row.get("bytes_out") or session_meta.get("bytes_out") or 0)
        bytes_total = bytes_in + bytes_out
        download_label = (
            usage_row.get("download_label")
            or session_meta.get("download_label")
            or (_usage_bps_label(download_bps) if download_bps is not None else "—")
        )
        upload_label = (
            usage_row.get("upload_label")
            or session_meta.get("upload_label")
        ...
        uplink_index = next(
            (idx for idx, name in enumerate(member_ports) if name == isp_port),
            None,
        )
        uplink_tracked = online and source == "connection_mark"
        if isp_pinned and online:
            uplink_source_label = "Pinned ISP"
        elif online and source == "connection_mark":
            uplink_source_label = "Live routing"
        elif online and source == "default_wan":
            uplink_source_label = "Default route"
        elif online and source == "session":
            uplink_source_label = "Active session"
        elif online:
            uplink_source_label = "Estimated"
        else:
            uplink_source_label = ""
        if isp_port and isp_port in isp_by_port and online:
            ...
            isp_by_port[isp_port]["online_clients"] = (
                int(isp_by_port[isp_port].get("online_clients") or 0) + 1
            )
            isp_by_port[isp_port]["connection_count"] = (
                int(isp_by_port[isp_port].get("connection_count") or 0) + connections
            )
            isp_by_port[isp_port]["download_bps_clients"] = int(
                isp_by_port[isp_port].get("download_bps_clients") or 0
            ) + int(download_bps or 0)
            isp_by_port[isp_port]["upload_bps_clients"] = int(
                isp_by_port[isp_port].get("upload_bps_clients") or 0
            ) + int(upload_bps or 0)
            isp_by_port[isp_port]["bytes_total"] = int(
                isp_by_port[isp_port].get("bytes_total") or 0
            ) + bytes_total
        lan_row = _ensure_lan_port(lan_port) if lan_port else None
        if lan_row is not None:
            lan_row["client_count"] = int(lan_row.get("client_count") or 0) + 1
            if online:
                lan_row["online_clients"] = int(lan_row.get("online_clients") or 0) + 1
                lan_row["connection_count"] = (
                    int(lan_row.get("connection_count") or 0) + connections
                )
                lan_row["download_bps"] = int(lan_row.get("download_bps") or 0) + int(
                    download_bps or 0
                )
                lan_row["upload_bps"] = int(lan_row.get("upload_bps") or 0) + int(
                    upload_bps or 0
                )
                lan_row["bytes_total"] = int(lan_row.get("bytes_total") or 0) + bytes_total
        client_rows.append(
            {
                ...
                "isp_port": isp_port,
                "isp_label": isp_label,
                "uplink_port": isp_port,
                "uplink_label": isp_label,
                "uplink_index": uplink_index,
                "uplink_tracked": uplink_tracked,
                "uplink_source_label": uplink_source_label,
                "isp_pinned": isp_pinned,
                "can_switch_isp": bool(can_switch_clients and online and ip),
                "lan_port": lan_port,
                "connection_count": connections,
                "assignment_source": source or ("offline" if not online else "default_wan"),
                "download_bps": download_bps,
                "upload_bps": upload_bps,
                "download_label": download_label if online else "—",
                "upload_label": upload_label if online else "—",
                "bytes_in": bytes_in if online else 0,
                "bytes_out": bytes_out if online else 0,
                "bytes_total": bytes_total if online else 0,
                "data_label": _usage_bytes_label(bytes_total) if online and bytes_total else "—",
    ...
    for row in lan_ports_map.values():
        down = int(row.get("download_bps") or 0)
        up = int(row.get("upload_bps") or 0)
        total = down + up
        row["download_label"] = _usage_bps_label(down) if down else "—"
        row["upload_label"] = _usage_bps_label(up) if up else "—"
        row["rate_label"] = _usage_bps_label(total) if total else "—"
        bytes_total = int(row.get("bytes_total") or 0)
        row["data_label"] = _usage_bytes_label(bytes_total) if bytes_total else "—"

    for row in isps:
        client_down = int(row.get("download_bps_clients") or 0)
        client_up = int(row.get("upload_bps_clients") or 0)
        if row.get("download_bps") is None and client_down:
            row["download_bps"] = client_down
            row["download_label"] = _usage_bps_label(client_down)
        if row.get("upload_bps") is None and client_up:
            row["upload_bps"] = client_up
            row["upload_label"] = _usage_bps_label(client_up)
        bytes_total = int(row.get("bytes_total") or 0)
        row["data_label"] = _usage_bytes_label(bytes_total) if bytes_total else "—"

    lan_ports = sorted(
        lan_ports_map.values(),
        key=lambda row: (
            0 if int(row.get("online_clients") or 0) else 1,
            -(int(row.get("download_bps") or 0) + int(row.get("upload_bps") or 0)),
            (row.get("port") or "").lower(),
        ),
    )
    ...
    # Group clients under each port for the UI (ISP + LAN).
    clients_by_isp: dict[str, list] = {row["port"]: [] for row in isps}
    clients_by_lan: dict[str, list] = {row["port"]: [] for row in lan_ports}
    unmapped_clients: list = []
    for client in client_rows:
        isp_port = (client.get("isp_port") or "").strip()
        lan_port = (client.get("lan_port") or "").strip()
        placed = False
        if lan_port and lan_port in clients_by_lan:
            clients_by_lan[lan_port].append(client)
            placed = True
        if isp_port and isp_port in clients_by_isp:
            clients_by_isp[isp_port].append(client)
            placed = True
        if not placed:
            unmapped_clients.append(client)

    for row in isps:
        row["clients"] = clients_by_isp.get(row["port"]) or []
    for row in lan_ports:
        row["clients"] = clients_by_lan.get(row["port"]) or []
    ...
    total_download = sum(int(row.get("download_bps") or 0) for row in client_rows if row.get("online"))
    total_upload = sum(int(row.get("upload_bps") or 0) for row in client_rows if row.get("online"))
    total_session_bytes = sum(
        int(row.get("bytes_total") or 0) for row in client_rows if row.get("online")
    )
    ...
    return {
        ...
        "summary": {
            ...
            "download_label": _usage_bps_label(total_download) if total_download else "—",
            "upload_label": _usage_bps_label(total_upload) if total_upload else "—",
            "session_data_label": (
                _usage_bytes_label(total_session_bytes) if total_session_bytes else "—"
            ),
            "lan_ports_with_clients": sum(
                1 for row in lan_ports if int(row.get("client_count") or 0) > 0
            ),
        },
        "clients": client_rows,
        "unmapped_clients": unmapped_clients,
        "can_switch_clients": can_switch_clients,
        "isp_switch_options": [
            {
                "port": row.get("port") or "",
                "label": row.get("label") or row.get("port") or "",
                "isp_index": row.get("isp_index"),
                "status": row.get("status") or "active",
                "disabled": (row.get("status") or "") in {"slow", "sidelined"},
            }
            for row in isps
            if (row.get("port") or "").strip()
        ],
    }
...
def _build_client_balance_insights(
    ...
    *,
    ...
    mode = (uplink_mode or MikroTikRouter.UplinkMode.SINGLE).strip()
    isps = list(analysis.get("isps") or [])
    clients = [c for c in (analysis.get("clients") or []) if c.get("online")]
    online_by_isp: dict[str, int] = {}
    for client in clients:
        port_name = (client.get("isp_port") or "").strip()
        if not port_name:
            continue
    ...
    verified_members = [
        ...
        for row in isps
        if (row.get("port") or "").strip()
        and _port_has_verified_internet(str(row.get("port")), physical_ports)
    ]
    ...
    ready_for_balance = balance_ready or (
        mode == MikroTikRouter.UplinkMode.BOND and len(verified_members) >= 2
    )
    can_auto_enable = bool(
        len(verified_members) >= 2
        and ready_for_balance
        and not applied
        and mode_can_balance
    )

    recommendations: list[str] = []
    if member_count < 2:
        recommendations.append(
            "Need two ISP links on this router before clients can balance across uplinks."
        )
        ...
        recommendations.append(
            "Clients are tracked per ISP link — live share updates every few seconds."
        )

    unmapped = analysis.get("unmapped_clients") or []
    if unmapped:
        recommendations.append(
            f"{len(unmapped)} online client(s) could not be mapped to a cable port yet."
        )
...
def _client_isp_rebalance_cache_key(router_pk: int) -> str:
    return f"client_isp_rebalance:{router_pk}"


def _try_auto_rebalance_client_isps(
    router: MikroTikRouter,
    api_host: str,
    *,
    balance_insights: dict,
...
def _assigned_balance_auto_cache_key(router_pk: int) -> str:
    return f"assigned_balance_auto:{router_pk}"


def _try_auto_enable_client_balance(
    router: MikroTikRouter,
    api_host: str,
    *,
    balance_insights: dict,
    ...
    """Background smart-balance apply when assigned-ports detects ready multi-ISP."""
    if not balance_insights.get("can_auto_enable"):
        return {}
    cache_key = _assigned_balance_auto_cache_key(router.pk)
...
def _wan_share_ports_with_traffic(wan_share: dict | None) -> set[str]:
    """Ports currently carrying measurable WAN traffic (live share sample)."""
    out: set[str] = set()
    if not isinstance(wan_share, dict) or not wan_share.get("ok"):
        return out
    total_bps = int(wan_share.get("total_bps") or 0)
    for row in wan_share.get("shares") or []:
...
def _uplink_live_isp_ready(port_name: str, uplink_live: dict | None) -> bool:
    """True when RouterOS reports a bound DHCP or running PPPoE client on the port."""
    if not isinstance(uplink_live, dict) or not uplink_live.get("ok"):
        return False
    port_name = (port_name or "").strip()
    if not port_name:
        return False
    for client in uplink_live.get("failover_clients") or []:
        if client.get("disabled"):
            continue
        ...
        if (client.get("kind") or "").strip().lower() == "pppoe":
            return status == "running"
        return status in {"bound", "connected", "running"}
    return False


def _port_isp_ready_for_uplink(
    port_name: str,
    physical_ports: list[dict],
    *,
    uplink_live: dict | None = None,
    wan_share: dict | None = None,
) -> bool:
    """
    Whether a WAN port is safe to treat as ISP-ready for apply / status.

    Uses port rows first, then live DHCP/PPPoE on the router, then active
    traffic share — avoids blocking apply when traffic is already flowing.
    """
    if _port_has_verified_internet(port_name, physical_ports):
        return True
...
def _guard_touch_ports_internet(
    ...
    *,
    ...
    """Block uplink apply when target ports lack live ISP internet."""
    ...
    for name in touch:
        if not _port_has_live_uplink(name, live_ports):
            down.append(name)
            continue
        if _port_isp_ready_for_uplink(
            name,
            live_ports,
            uplink_live=uplink_live,
            wan_share=wan_share,
            ...
            continue
        row = by_name.get(name) or {"name": name}
        inet = assess_port_internet_readiness(row)
        msg = (inet.get("message") or "").strip() or (
            f"{name} has no verified internet."
        )
        no_inet.append(msg)
    if down:
        return False, f"Link must be up: {', '.join(down)}."
    if no_inet:
        return False, no_inet[0]
    return True, ""


def _wan_share_snapshot_for_ports(
    router: MikroTikRouter,
    live_ports: list[dict],
    port_names: list[str],
) -> dict:
    """Best-effort live traffic share for apply guards (uses cached byte deltas)."""
    names = [str(p).strip() for p in (port_names or []) if str(p).strip()]
    if len(names) < 2:
        return {"ok": False, "shares": [], "total_bps": 0}
    by_name = {
        (p.get("name") or "").strip(): p for p in live_ports if (p.get("name") or "").strip()
    }
    samples = []
    for name in names:
        row = by_name.get(name) or {}
    ...
    try:
        previous = cache.get(f"mikrotik_wan_bytes:{router.pk}")
        share, _next = build_wan_traffic_share(
            samples, previous=previous if isinstance(previous, dict) else None
        )
        return share if isinstance(share, dict) else {"ok": False, "shares": [], "total_bps": 0}
    except Exception:
        return {"ok": False, "shares": [], "total_bps": 0}


def _read_uplink_live_for_apply(
    router: MikroTikRouter,
    api_host: str,
    member_ports: list[str],
) -> dict:
    """Read RouterOS multi-uplink state for apply guards (non-fatal on failure)."""
    try:
        return read_mikrotik_uplink_multi(
            api_host,
            router.username,
            router.password or "",
            timeout=8.0,
            member_ports=list(member_ports or []),
        )
    except Exception:
        return {"ok": False}


def _apply_internet_guard_context(
    router: MikroTikRouter,
    api_host: str,
    live_ports: list[dict],
    touch_ports: list[str],
) -> tuple[dict, dict]:
    """Uplink live + traffic share context shared by apply POST handlers."""
    ordered = [str(p).strip() for p in (touch_ports or []) if str(p).strip()]
    uplink_live = _read_uplink_live_for_apply(router, api_host, ordered)
    wan_share = _wan_share_snapshot_for_ports(router, live_ports, ordered)
    return uplink_live, wan_share
...
def _port_bridge_warn_payload(
    ...
    *,
    ...
    """Role-aware bridge note for uplink ports still on the LAN bridge."""
    if not row.get("is_bridged"):
        return {"show": False, "message": "", "level": ""}
    role = (role or "").strip().lower()
    suggested = (suggested_role or "").strip().lower()
    uplink_roles = {
        MikroTikRouter.PortRole.WAN,
        MikroTikRouter.PortRole.WAN_PRIMARY,
        MikroTikRouter.PortRole.WAN_BACKUP,
    ...
    active = role if role in uplink_roles else suggested
    if active not in uplink_roles:
        return {"show": False, "message": "", "level": ""}
    bridge = (row.get("bridge") or "LAN").strip() or "LAN"
    mode = (uplink_mode or "").strip()
    if active == MikroTikRouter.PortRole.WAN_BACKUP or (
        _is_multi_isp_mode(mode) and active in {MikroTikRouter.PortRole.WAN, MikroTikRouter.PortRole.WAN_PRIMARY}
        and role == MikroTikRouter.PortRole.WAN_BACKUP
    ):
        return {
            "show": True,
            "level": "info",
            "message": (
                f"On {bridge} — customers stay online until you Apply multi-ISP. "
                "This cable unbridges once Shared ISP gets DHCP/PPPoE."
            ),
        }
    if active == MikroTikRouter.PortRole.BOND:
        return {
            "show": True,
            "level": "warn",
            "message": (
                f"On {bridge} — bond members must leave the bridge before Apply. "
                "Customer ports on other bridge members stay connected."
            ),
        }
    return {
        "show": True,
        "level": "warn",
        "message": (
            f"On {bridge} — kept bridged for customers until Internet is confirmed "
            "on this port."
        ),
    }


def _shared_isp_probe_cache_key(router_pk: int, port_name: str) -> str:
    return f"shared_isp_probe:{router_pk}:{(port_name or '').strip()}"


def _try_probe_shared_isp_ports(
    router: MikroTikRouter,
    api_host: str,
    live_ports: list[dict],
) -> dict:
    """
    Start DHCP on Shared ISP ports that are still on the LAN bridge.

    Unbridges only the target port (e.g. ether2) so customer ports on ether3+
    stay on the bridge. Does not add a default route — ether1 keeps Internet.
    """
    mode = (router.uplink_mode or "").strip()
    if not _is_multi_isp_mode(mode):
        return {}
    if active_uplink_apply_job(router.pk):
        return {}

    primary, backups = _failover_ports_from_roles(router)
    if not primary or not backups:
        return {}
    if not _port_has_verified_internet(primary, live_ports):
        return {}
    ...
    for name in backups:
        ...
        if not name:
            continue
        row = by_name.get(name) or {}
        if _port_has_verified_internet(name, live_ports):
            cache.set(_shared_isp_probe_cache_key(router.pk, name), "bound", 600)
            continue
        if row.get("disabled") or not row.get("running"):
            continue
        ...
        if row.get("is_bridged"):
            blocked = _reject_behind_provider_unbridge(
                [name],
                live_ports,
                mode_label="Shared ISP probe",
            )
            if blocked:
                continue

        cache.set(cache_key, "pending", 90)
            ...
            if unbridged:
                ...
                for entry in unbridged:
                    ...
                    if iface and iface not in seen:
                        stored.append(entry)
                        seen.add(iface)
                router.uplink_unbridged = stored
                router.save(update_fields=["uplink_unbridged", "updated_at"])
            outcomes[name] = result
        else:
            cache.set(cache_key, "failed", 120)
            outcomes[name] = result
    return outcomes


def _port_shows_internet_status(role: str) -> bool:
    """Only uplink roles show ISP online / pending badges on port cards."""
    return (role or "").strip().lower() in {
        MikroTikRouter.PortRole.WAN,
        MikroTikRouter.PortRole.WAN_PRIMARY,
        MikroTikRouter.PortRole.WAN_BACKUP,
...
def _bond_router_applied(
    uplink_mode: str,
    uplink_live: dict | None,
    *,
    bond_member_ports: list[str] | None = None,
    bond_interface: str = "",
) -> bool:
    """True when a tagged bond on the router matches this setup (members + name)."""
    if (uplink_mode or "").strip() != MikroTikRouter.UplinkMode.BOND:
        return False
    live = uplink_live if isinstance(uplink_live, dict) else {}
    if not live.get("ok"):
        return False
    bonds = live.get("bonds") or []
    if not bonds:
        return False
    bond = bonds[0] or {}
    if not bond.get("running") or bond.get("disabled"):
        return False
    bname = (bond_interface or "").strip()
    if bname and (bond.get("name") or "").strip() != bname:
        return False
    members = sorted(str(p).strip() for p in (bond_member_ports or []) if str(p).strip())
    if len(members) >= 2:
        slaves = sorted(
            str(s).strip() for s in (bond.get("slaves") or []) if str(s).strip()
        )
        if slaves != members:
            return False
    return True
    ...
    *,
    ...
    for name in primary_wan_ports + backup_wan_ports:
        if not _port_has_live_uplink(name, physical_ports):
            down.append(name)
        elif not _port_isp_ready_for_uplink(
            name,
            physical_ports,
            uplink_live=uplink_live,
            wan_share=wan_share,
...
def _ether_port_number(name: str) -> int | None:
    """Numeric suffix for ether1, ether2, … (None for non-ether names)."""
    name = (name or "").strip().lower()
    if not name.startswith("ether"):
        return None
...
def _ether_sort_key(name: str) -> tuple:
    num = _ether_port_number(name)
    return (0 if num is not None else 1, num or 0, (name or "").lower())


def _physical_manageable_ports(live_ports: list[dict]) -> list[dict]:
    return [
        p
        for p in live_ports
        if not _is_bond_port_row(p)
        and not _is_bridge_port_name((p.get("name") or "").strip())
    ]


def _port_has_isp_signals(row: dict | None) -> bool:
    """Port has a live or configured ISP client (DHCP bound / PPPoE)."""
    row = row or {}
    kind = (row.get("uplink_kind") or "").strip().lower()
    if kind not in {"dhcp", "pppoe"}:
        return False
    if kind == "pppoe":
        return bool(row.get("uplink_active") or row.get("running"))
    return bool(row.get("uplink_active"))


def _sync_ports_internet_display(
    ports: list[dict],
    *,
    uplink_live: dict | None = None,
    wan_share: dict | None = None,
) -> None:
    """Align port-card ISP badges with live traffic and RouterOS DHCP state."""
    live = uplink_live if isinstance(uplink_live, dict) else {}
    share = wan_share if isinstance(wan_share, dict) else {}
    for port in ports:
        if not port.get("show_internet_status") and not port.get("is_bond_iface"):
            continue
        if (port.get("role") or "").strip() == MikroTikRouter.PortRole.BOND:
            continue
        name = (port.get("name") or "").strip()
        if not name:
            continue
        if port.get("failover_carrying"):
            port["internet_verified"] = True
            port["internet_hint"] = ""
            port["internet_level"] = "ok"
            continue
        if _port_isp_ready_for_uplink(
            name,
            ports,
...
def _sync_ports_failover_display(
    ports: list[dict],
    *,
    uplink_mode: str,
    primary_wan_ports: list[str],
    ...
    """
    Port failover badges: in load-balance / smart-balance both links route via
    PCC — do not mark the primary as "not routing" when it still has live share.
    """
    mode = (uplink_mode or "").strip()
    live = uplink_live if isinstance(uplink_live, dict) else {}
    share = wan_share if isinstance(wan_share, dict) else {}
    ...
    failover_active = ""
    if _is_multi_isp_mode(mode) and live.get("ok"):
        failover_active = _failover_active_wan_port(
            live,
            primary_wan_ports=primary_wan_ports,
            backup_wan_ports=backup_wan_ports,
        )
    sharing = _wan_share_ports_with_traffic(share) if share.get("ok") else set()
    member_sharing = sharing & set(members)
    both_sharing = _is_weighted_share_mode(mode) and len(member_sharing) >= 2

    for port in ports:
        name = (port.get("name") or "").strip()
        port["sharing_traffic"] = bool(
            _is_weighted_share_mode(mode) and name in member_sharing
        )
        if both_sharing:
            port["failover_carrying"] = False
            port["failover_primary_stood_down"] = False
            continue
        if mode == MikroTikRouter.UplinkMode.FAILOVER or (
            _is_weighted_share_mode(mode) and len(member_sharing) <= 1
        ):
            port["failover_carrying"] = bool(failover_active and name == failover_active)
            port["failover_primary_stood_down"] = bool(
                name in primary_wan_ports
                and failover_active
                and failover_active not in primary_wan_ports
            )
        else:
            port["failover_carrying"] = False
            port["failover_primary_stood_down"] = False
...
def _infer_port_topology(
    live_ports: list[dict],
    *,
    suggested_wan: str = "",
    saved_wan: str = "",
    max_auto_uplinks: int = 2,
) -> dict:
    """
    Infer uplink vs customer ports from live link layout.

    Typical ISP router: ether1 + ether2 carry ISP cables; ether3+ are customer
    LAN on the bridge. Uses ISP signals first, then consecutive linked ether
    ports from the primary before the customer block.
    """
    physical = _physical_manageable_ports(live_ports)
    by_name = {(p.get("name") or "").strip(): p for p in physical}
    primary = _pick_auto_wan(
        physical, suggested_wan=suggested_wan, saved_wan=saved_wan
    )

    uplink_ordered: list[str] = []
    seen: set[str] = set()

    def add_uplink(name: str) -> None:
        name = (name or "").strip()
        ...
        if row.get("disabled") or row.get("is_wireless"):
            return
        seen.add(name)
        uplink_ordered.append(name)

    if primary:
        add_uplink(primary)

    for row in sorted(
        physical, key=lambda item: _ether_sort_key(item.get("name") or "")
    ):
        name = (row.get("name") or "").strip()
        if not name or name in seen:
            continue
        if _port_has_isp_signals(row) or _port_has_verified_internet(name, physical):
            add_uplink(name)

    if primary and len(uplink_ordered) < max_auto_uplinks:
        primary_num = _ether_port_number(primary)
        if primary_num is not None:
            ether_rows = sorted(
                (
                    r
                    for r in physical
                    if _ether_port_number(r.get("name") or "") is not None
                ),
                key=lambda item: _ether_port_number(item.get("name") or "") or 0,
            )
            for row in ether_rows:
                if len(uplink_ordered) >= max_auto_uplinks:
                    break
                name = (row.get("name") or "").strip()
                if not name or name in seen:
                    continue
                if not row.get("running") or row.get("disabled"):
                    continue
                num = _ether_port_number(name)
                if num is None:
                    continue
                block_max = max(
                    _ether_port_number(n) or 0 for n in uplink_ordered
                )
                if num != block_max + 1:
                    continue
                if row.get("is_bridged") and not _port_has_isp_signals(row):
                    # Bridged ISP cable (ether2) sits before linked customer ports (ether3+).
                    later_linked = any(
                        (_ether_port_number(r.get("name") or "") or 0) > num
                        and r.get("running")
    ...
    for row in physical:
        ...
        if row.get("is_bridged"):
            ...
            continue
        num = _ether_port_number(name)
        if row.get("running") and num is not None and num > block_max:
            customers.add(name)
        elif not row.get("running"):
            unused.add(name)

    return {
        "primary": primary or (uplink_ordered[0] if uplink_ordered else ""),
        "uplinks": uplink_ordered,
        "customers": sorted(customers, key=_ether_sort_key),
        "unused": sorted(unused, key=_ether_sort_key),
    }
...
def _live_isp_member_ports(
    ...
    *,
    ...
    physical = _physical_manageable_ports(live_ports)
    by_name = {(p.get("name") or "").strip(): p for p in physical}
    topology = _infer_port_topology(
        live_ports,
        suggested_wan=suggested_wan,
        saved_wan=saved_wan,
    )
    primary = (topology.get("primary") or "").strip() or _pick_auto_wan(
        physical,
        suggested_wan=suggested_wan,
        saved_wan=saved_wan,
    )
    ...
    def add(name: str, *, require_verified: bool = True) -> None:
        name = (name or "").strip()
        if not name or name in seen:
            return
        if require_verified and not _port_has_verified_internet(name, physical):
            return
        seen.add(name)
        members.append(name)
    ...
    # Soft members from topology (e.g. ether2 ISP cable linked, waiting for DHCP).
    if len(members) < 2:
        for name in topology.get("uplinks") or []:
            add(name, require_verified=False)
            if len(members) >= 2:
                break
    return members


def _uplink_candidate_status(row: dict) -> dict:
    """Operator-facing link status for recommendation chips."""
    readiness = assess_port_internet_readiness(row)
    kind = (row.get("uplink_kind") or "").strip().lower()
    kind_label = {"pppoe": "PPPoE", "dhcp": "DHCP"}.get(kind, "")
    verified = bool(readiness.get("verified"))
    running = bool(row.get("running")) and not row.get("disabled")
    if verified:
        status = "isp_online"
        status_label = "ISP online"
        if kind_label:
            status_label = f"ISP online ({kind_label})"
    elif kind == "dhcp" and running:
        status = "waiting_dhcp"
        status_label = "Waiting for DHCP"
    elif kind == "pppoe":
        status = "waiting_pppoe"
        status_label = "Waiting for PPPoE"
    elif running:
        status = "link_up"
        status_label = "Link up"
    elif row.get("disabled"):
        status = "disabled"
        status_label = "Disabled"
    else:
        status = "down"
        status_label = "No link"
    return {
        "status": status,
        "status_label": status_label,
        "verified": verified,
        "level": readiness.get("level") or ("ok" if verified else "warn"),
        "hint": (readiness.get("message") or "").strip(),
        "uplink_kind": kind,
        "uplink_kind_label": kind_label or "link",
        "running": running,
    }


def _score_uplink_candidate(
    row: dict,
    *,
    suggested_wan: str = "",
    saved_wan: str = "",
) -> int:
    """Higher score = better recommended Internet / Shared ISP member."""
    if not row or row.get("disabled") or row.get("is_wireless") or _is_bond_port_row(row):
        return -1
    name = (row.get("name") or "").strip()
    ...
    if meta["verified"]:
        ...
        if kind == "pppoe":
            ...
            score += 60
    elif meta["status"] == "waiting_pppoe":
        score += 420
    elif meta["status"] == "waiting_dhcp":
        score += 400
    elif meta["status"] == "link_up":
        score += 250
    if name == (suggested_wan or "").strip():
        score += 200
    if name == (saved_wan or "").strip():
        score += 50
    if row.get("running"):
        score += 20
    if row.get("is_bridged") and not row.get("uplink_active"):
        score -= 40
    # Prefer lower ether numbers slightly for stable ordering.
    digits = "".join(ch for ch in name if ch.isdigit())
    if digits:
        try:
            score -= min(15, int(digits))
        except ValueError:
            pass
    return score


def _list_uplink_candidates(
    live_ports: list[dict],
    *,
    suggested_wan: str = "",
    saved_wan: str = "",
) -> list[dict]:
    """Ranked live links (link up or ISP) for internet setup recommendations."""
    physical = _physical_manageable_ports(live_ports)
    topology = _infer_port_topology(
        live_ports, suggested_wan=suggested_wan, saved_wan=saved_wan
    )
    customer_ports = set(topology.get("customers") or [])
    out: list[dict] = []
    for row in physical:
        name = (row.get("name") or "").strip()
        if name in customer_ports:
            continue
        score = _score_uplink_candidate(
            row, suggested_wan=suggested_wan, saved_wan=saved_wan
        )
        if score < 0:
            continue
        name = (row.get("name") or "").strip()
        ...
        if name == (suggested_wan or "").strip():
            reason_bits.append("matches live default route")
        if meta["verified"]:
            reason_bits.append("ISP internet confirmed")
        elif meta["status"].startswith("waiting"):
            reason_bits.append("cable linked — waiting for ISP")
        elif meta["status"] == "link_up":
            reason_bits.append("cable linked")
        out.append(
            {
                "name": name,
                "score": score,
                "recommended_role": "",
                "reason": ", ".join(reason_bits) or meta["status_label"],
                **meta,
            }
        )
    out.sort(key=lambda item: (-int(item.get("score") or 0), item.get("name") or ""))
    return out


def _roles_match_recommendation(
    router: MikroTikRouter,
    *,
    mode: str,
    primary: str = "",
    ...
    mode = (mode or "").strip()
    if mode == MikroTikRouter.UplinkMode.SINGLE:
        return _current_primary_wan_port(router) == (primary or "").strip()
    if mode == MikroTikRouter.UplinkMode.BOND:
        current = _bond_ports_from_roles(router)
        want = [str(p).strip() for p in (members or []) if str(p).strip()]
        return sorted(current) == sorted(want) and len(want) >= 2
    cur_primary, cur_backups = _failover_ports_from_roles(router)
    want_backups = [str(p).strip() for p in (backups or []) if str(p).strip()]
    return cur_primary == (primary or "").strip() and cur_backups == want_backups


def _build_uplink_recommendation(
    router: MikroTikRouter,
    live_ports: list[dict],
    *,
    suggested_wan: str = "",
    dual_wan_ready: bool = False,
    bond_ready: bool = False,
    ...
    """
    Auto-identify live links and recommend the best internet setup for the
    selected goal. User can accept to auto-manage roles; status says when
    apply / advance is safe.
    """
    mode = (router.uplink_mode or MikroTikRouter.UplinkMode.SINGLE).strip()
    saved_wan = (router.wan_interface or "").strip()
    candidates = _list_uplink_candidates(
        live_ports, suggested_wan=suggested_wan, saved_wan=saved_wan
    )
    ...
    empty = {
        ...
        "phase": "watching",
        "level": "info",
        "title": "Watching live links",
        "message": "Plug in ISP cables — recommendations appear when links come up.",
        "accept_label": "Use recommendation",
        "waiting_ports": [c["name"] for c in waiting],
    }

    if mode == MikroTikRouter.UplinkMode.SINGLE:
        pick = ""
        if verified:
            pick = verified[0]["name"]
        else:
            pick = _pick_auto_wan(
                live_ports, suggested_wan=suggested_wan, saved_wan=saved_wan
            )
        ...
        for row in candidates:
            if row["name"] == pick:
                row["recommended_role"] = "Internet"
        accepted = bool(pick and current == pick)
        ready = bool(pick and _port_has_verified_internet(pick, live_ports))
        can_accept = bool(pick and not accepted)
        if not pick and waiting:
            wait_names = ", ".join(c["name"] for c in waiting[:3])
            return {
                **empty,
                "candidates": candidates,
                "waiting_ports": [c["name"] for c in waiting],
                "phase": "waiting",
                "level": "info",
                "title": "Link detected — waiting for ISP",
                "message": (
                    f"{wait_names} is linked. Wait for DHCP/PPPoE, then we will "
                    "recommend the Internet port."
                ),
            }
        if not pick:
            return empty
        multi_hint = ""
        if len(waiting) >= 1 and ready:
            wait_names = ", ".join(c["name"] for c in waiting[:3])
            multi_hint = (
                f" Extra ISP on {wait_names} — use Set up multiple ISPs above "
                "for share and automatic failover."
            )
        if accepted and ready:
            phase, level, title = "ready", "ok", "Internet link ready"
            message = (
                f"{pick} is the recommended Internet port and is ISP online. "
                "You can advance — change ports anytime."
                + multi_hint
            )
        elif accepted and not ready:
            phase, level, title = "waiting", "info", "Waiting for ISP on Internet port"
            message = (
                f"{pick} is selected as Internet but ISP is not confirmed yet — "
                "wait for DHCP/PPPoE before relying on this link."
            )
        elif ready:
            phase, level, title = "recommend", "info", "Recommended Internet port"
            message = (
                f"Use {pick} as Internet "
                f"({next((c['status_label'] for c in candidates if c['name'] == pick), 'ISP online')}). "
                "Select it to auto-label and manage the link."
            )
        else:
            phase, level, title = "waiting", "info", "Recommended after ISP comes online"
            message = (
                f"{pick} looks like the Internet cable (link up). "
                "Wait for DHCP/PPPoE, then select the recommendation."
            )
            can_accept = False
        return {
            **empty,
            "candidates": candidates,
            "recommended_ports": [pick],
            "primary": pick,
            "members": [pick],
            "accepted": accepted,
            "can_accept": can_accept,
            "auto_managed": accepted,
            "ready_to_apply": ready and accepted,
            "can_proceed": ready and accepted,
            "phase": phase,
            "level": level,
            "title": title,
            "message": message,
            "accept_label": f"Use {pick} as Internet",
            "waiting_ports": [c["name"] for c in waiting],
        }
    ...
    if mode == MikroTikRouter.UplinkMode.BOND:
        ...
        if len(members) < 2:
            members = _live_bond_candidate_ports(live_ports)[:2]
        if len(members) < 2 and len(waiting) >= 2:
            members = [c["name"] for c in waiting[:2]]
        for row in candidates:
            if row["name"] in members:
                row["recommended_role"] = "Bonded internet"
        accepted = _roles_match_recommendation(
            router, mode=mode, members=members
        )
        if bond_router_applied and len(members) >= 2:
            bond_name = (router.bond_interface or "bond-wan").strip() or "bond-wan"
            return {
                **empty,
                "candidates": candidates,
                "recommended_ports": members,
                "members": members,
                "accepted": True,
                "can_accept": False,
                "auto_managed": True,
                ...
                "phase": "applied",
                "level": "ok",
                "title": "Bonding active",
                "message": (
                    f"{bond_name} is combining {' + '.join(members)} on the MikroTik."
                ),
                "accept_label": f"Use {' + '.join(members)}",
                "waiting_ports": [],
            }
        ready = bool(bond_ready and accepted)
        can_accept = len(members) >= 2 and not accepted
        if len(members) < 2:
            return {
                **empty,
                "candidates": candidates,
                "phase": "watching",
                "level": "info",
                "title": "Waiting for two linked cables",
                "message": (
                    "Bonding needs two cables from the same ISP. "
                    "Plug them in — we will recommend the pair automatically."
                ),
                "waiting_ports": [c["name"] for c in waiting],
            }
        if accepted and ready:
            phase, level, title = "ready_to_apply", "ok", "Ready to apply bonding"
            message = (
                f"{' + '.join(members)} look ready. Apply bonding to combine them."
            )
        elif accepted:
            phase, level, title = "waiting", "info", "Bond members labeled"
            message = (
                f"{' + '.join(members)} are marked for bonding — "
                "confirm both links stay up, then apply."
            )
        else:
            phase, level, title = "recommend", "info", "Recommended bond pair"
            message = (
                f"Use {' + '.join(members)} as bonded internet. "
                "Select to auto-label both ports."
            )
        return {
            **empty,
            "candidates": candidates,
            ...
            "can_proceed": ready or (
                accepted and balance_router_applied is False and bond_ready
            ),
            "phase": phase,
            "level": level,
            "title": title,
            "message": message,
            "accept_label": f"Use {' + '.join(members)}",
            "waiting_ports": [c["name"] for c in waiting if c["name"] not in members],
        }

    # Multi-ISP (failover / balance / smart_balance)
    topology = _infer_port_topology(
        live_ports, suggested_wan=suggested_wan, saved_wan=saved_wan
    )
    member_names = _live_isp_member_ports(
        live_ports, suggested_wan=suggested_wan, saved_wan=saved_wan
    )
    if len(member_names) < 2 and len(topology.get("uplinks") or []) >= 2:
        member_names = list(topology["uplinks"])
    if len(member_names) < 2 and verified:
        # Prefer verified primary + best waiting as future pair for messaging.
        primary_name = verified[0]["name"]
        member_names = [primary_name] + [
            c["name"] for c in verified[1:] if c["name"] != primary_name
        ]
    primary = member_names[0] if member_names else ""
    backups = member_names[1:] if len(member_names) > 1 else []
    for row in candidates:
        if row["name"] == primary:
            row["recommended_role"] = "Internet"
        elif row["name"] in backups:
            row["recommended_role"] = "Shared ISP"
        elif row["name"] in {c["name"] for c in waiting}:
            row["recommended_role"] = "Shared ISP (pending)"
    accepted = bool(
        primary
        and backups
        and _roles_match_recommendation(
            router, mode=mode, primary=primary, backups=backups
        )
    )
    applied = bool(balance_router_applied) and (
        mode != MikroTikRouter.UplinkMode.SMART_BALANCE
    ...
    ready = bool(balance_ready or dual_wan_ready) and accepted

    if len(verified) >= 2:
        can_accept = not accepted
        if accepted and applied and ready:
            phase, level, title = "applied", "ok", "Multi-ISP ready — you can advance"
            message = (
                f"{primary} + {' + '.join(backups)} are healthy. "
                "Share + failover are active. You can proceed."
            )
        elif accepted and ready:
            phase, level, title = "ready_to_apply", "ok", "Ready to apply multi-ISP"
            message = (
                f"Recommended links are labeled ({primary} Internet, "
                f"{', '.join(backups)} Shared ISP). Apply to share traffic "
                "with failover on every link."
            )
        elif accepted:
            phase, level, title = "waiting", "info", "Links labeled — confirming ISP"
            message = (
                "Roles follow the recommendation. Wait until every Shared ISP "
                "shows ISP online, then apply."
            )
        else:
            phase, level, title = "recommend", "info", "Recommended ISP links"
            message = (
                f"Use {primary} as Internet and {', '.join(backups)} as Shared ISP "
                "(best live connections). Select to auto-manage labels."
            )
        return {
            **empty,
            "candidates": candidates,
            "recommended_ports": [primary, *backups],
            "primary": primary,
            "backups": backups,
            "members": [primary, *backups],
            "accepted": accepted,
            "can_accept": can_accept,
            "auto_managed": accepted,
            "ready_to_apply": ready and not applied,
            "can_proceed": (ready and not applied) or (applied and ready),
            "phase": phase,
            "level": level,
            "title": title,
            "message": message,
            "accept_label": (
                f"Use {primary} + {' + '.join(backups)}"
                if backups
                else f"Use {primary}"
            ),
            "waiting_ports": [c["name"] for c in waiting],
    ...
    if len(verified) == 1 and waiting:
        wait_names = ", ".join(c["name"] for c in waiting[:3])
        return {
            **empty,
            "candidates": candidates,
            "recommended_ports": [verified[0]["name"], waiting[0]["name"]],
            "primary": verified[0]["name"],
            "backups": [],
            "members": [verified[0]["name"]],
            "phase": "waiting",
            "level": "info",
            "title": "Almost ready — waiting for second ISP",
            "message": (
                f"{verified[0]['name']} is ISP online (recommended Internet). "
                f"{wait_names} is linked but not verified yet — wait for DHCP/PPPoE. "
                "We will auto-update the recommendation when it is ready."
            ),
            "waiting_ports": [c["name"] for c in waiting],
        }

    if len(waiting) >= 2:
        names = [c["name"] for c in waiting[:4]]
        return {
            **empty,
            "candidates": candidates,
            "recommended_ports": names[:2],
            "phase": "waiting",
            "level": "info",
            "title": "Links up — waiting for ISP on each cable",
            "message": (
                f"{' + '.join(names[:2])} are linked. "
                "Wait for DHCP/PPPoE on both before selecting the recommendation "
                "or applying multi-ISP (otherwise you may lose connection)."
            ),
            "waiting_ports": names,
        }
    ...
    if len(verified) == 1:
        return {
            **empty,
            "candidates": candidates,
            "recommended_ports": [verified[0]["name"]],
            "primary": verified[0]["name"],
            "members": [verified[0]["name"]],
            "phase": "watching",
            "level": "info",
            "title": "Need a second ISP link",
            "message": (
                f"{verified[0]['name']} is online. Plug in another ISP cable — "
                "we will recommend Internet + Shared ISP automatically."
            ),
        }
...
def apply_uplink_recommendation(
    router: MikroTikRouter,
    live_ports: list[dict],
    *,
    suggested_wan: str = "",
) -> dict:
    """Apply the current goal's recommended port roles (auto-manage labels)."""
    mode = (router.uplink_mode or MikroTikRouter.UplinkMode.SINGLE).strip()
    recommendation = _build_uplink_recommendation(
        router,
        live_ports,
        suggested_wan=suggested_wan,
    )
    if not recommendation.get("can_accept"):
        return {
            "ok": False,
            "error": recommendation.get("message")
            or "No recommendation is ready to accept yet.",
            "recommendation": recommendation,
        }

    if mode == MikroTikRouter.UplinkMode.SINGLE:
        port_name = (recommendation.get("primary") or "").strip()
        if not port_name:
            return {"ok": False, "error": "No Internet port to apply."}
        if not _port_has_verified_internet(port_name, live_ports):
            return {
                "ok": False,
                "error": (
                    f"{port_name} is not ISP online yet — wait for DHCP/PPPoE."
                ),
            }
    ...
    if mode == MikroTikRouter.UplinkMode.BOND:
        members = [
            str(p).strip() for p in (recommendation.get("members") or []) if str(p).strip()
        ]
        if len(members) < 2:
            return {"ok": False, "error": "Need two linked ports for bonding."}
        roles = dict(router.port_roles) if isinstance(router.port_roles, dict) else {}
        by_name = {
            (p.get("name") or "").strip(): p
            for p in live_ports
        ...
        roles = _fill_non_uplink_customer_roles(
            roles, live_ports, uplink_names=members
        )
        ...
        return {
            ...
            "members": members,
            "message": (
                f"Using recommended bond pair {' + '.join(members)}. "
                "Apply bonding when ready."
            ),
            "recommendation": recommendation,
        }

    # Multi-ISP
    primary = (recommendation.get("primary") or "").strip()
    backups = [
        str(p).strip() for p in (recommendation.get("backups") or []) if str(p).strip()
    ]
    members = [primary, *backups] if primary else []
    if len(members) < 2:
        return {
            "ok": False,
            "error": "Need two ISP-online links before applying the recommendation.",
        }
    for name in members:
        if not _port_has_verified_internet(name, live_ports):
            return {
                "ok": False,
                "error": (
                    f"{name} is not ISP online yet — wait for DHCP/PPPoE before "
                    "accepting the recommendation."
                ),
            }
    assign = _auto_assign_multi_isp_roles(
        router,
        live_ports,
        mode=mode,
        suggested_wan=suggested_wan,
    ...
    if not assign.get("ok"):
        return {
            "ok": False,
            "error": assign.get("error") or "Could not apply multi-ISP recommendation.",
            "recommendation": recommendation,
        }
    return {
        "ok": True,
        "changed": bool(assign.get("changed")),
        "primary": assign.get("primary") or primary,
        "backups": assign.get("backups") or backups,
        "message": assign.get("message")
        or (
            f"Using recommended ISP links: {primary} (Internet) + "
            f"{', '.join(backups)} (Shared ISP)."
        ),
        "recommendation": recommendation,
    }
...
def _auto_assign_multi_isp_roles(
    ...
    *,
    ...
    if len(members) < 2:
        ...
        topo_uplinks = list(topology.get("uplinks") or [])
        soft_primary = (primary or (verified[0] if verified else "")).strip()
        if not soft_primary and topo_uplinks:
            soft_primary = topo_uplinks[0]
        soft_backup = ""
        if len(topo_uplinks) >= 2 and topo_uplinks[0] == soft_primary:
            soft_backup = topo_uplinks[1]
        elif soft_primary and waiting:
            soft_backup = next(
...
def _fill_non_uplink_customer_roles(
    roles: dict[str, str],
    live_ports: list[dict],
    *,
    uplink_names: set[str] | list[str],
) -> dict[str, str]:
    """Label non-uplink ports as Customers / Unused from live link state."""
    out = dict(roles or {})
    uplink = {(n or "").strip() for n in (uplink_names or []) if (n or "").strip()}
    for row in live_ports:
        ...
        if row.get("is_bridged") or row.get("is_wireless") or row.get("running"):
            ...
            # Keep an explicit Unused; otherwise leave quiet ports unused.
            existing = (out.get(name) or "").strip().lower()
            if existing in {
                MikroTikRouter.PortRole.WAN,
                MikroTikRouter.PortRole.WAN_PRIMARY,
                MikroTikRouter.PortRole.WAN_BACKUP,
                ...
                out[name] = MikroTikRouter.PortRole.UNUSED
            elif existing in {"", MikroTikRouter.PortRole.NONE}:
                out[name] = MikroTikRouter.PortRole.UNUSED
    return out


def _roles_maps_equal(a: dict[str, str], b: dict[str, str]) -> bool:
    """Compare role maps ignoring blank / none equivalence."""
    keys = set(a or {}) | set(b or {})
    for key in keys:
        left = (a.get(key) or "").strip().lower()
        right = (b.get(key) or "").strip().lower()
        if left in {"", MikroTikRouter.PortRole.NONE}:
            left = MikroTikRouter.PortRole.NONE
        if right in {"", MikroTikRouter.PortRole.NONE}:
            right = MikroTikRouter.PortRole.NONE
        if left != right:
            return False
    return True


def _live_poll_should_apply_on_router(
    router: MikroTikRouter,
    live_ports: list[dict],
    *,
    suggested_wan: str = "",
) -> bool:
    """Allow debounced single-WAN RouterOS push on live poll only after a real outage.

    Healthy primary links stay label-only (sticky WAN). When the Internet port loses
    verified ISP and another port has confirmed internet, the existing 4-poll debounce
    in ``_auto_assign_single_wan_roles`` may push the switch without operator action.
    """
    mode = (router.uplink_mode or MikroTikRouter.UplinkMode.SINGLE).strip()
    if mode != MikroTikRouter.UplinkMode.SINGLE:
        return False
    ...
    primary = _current_primary_wan_port(router)
    if primary and _port_has_verified_internet(primary, live_ports):
        return False

    pending = cache.get(f"mikrotik_wan_pending:{router.pk}")
    if isinstance(pending, dict) and (pending.get("to") or "").strip():
        pick = (pending.get("to") or "").strip()
        if pick and _port_has_verified_internet(pick, live_ports):
            return True
    ...
    if not pick:
        return False
    if primary and pick == primary:
        return False
    return _port_has_verified_internet(pick, live_ports)
...
def _auto_assign_single_wan_roles(
    ...
    *,
    suggested_wan: str = "",
    api_host: str = "",
    apply_on_router: bool = False,
) -> dict:
    """
    Keep single-WAN labels in sync with live ISP / bridge state.

    Sticky rule: if the current Internet port still has verified ISP, keep it.
    That stops live-poll flaps (suggested_wan bouncing) from repeatedly
    reconfiguring RouterOS and dropping customers.
    """
    roles = dict(router.port_roles) if isinstance(router.port_roles, dict) else {}
    old_wan = next(
        (name for name, value in roles.items() if _is_primary_wan_role(value)),
        (router.wan_interface or "").strip(),
    )
    old_wan_ok = bool(
        old_wan and _port_has_verified_internet(old_wan, live_ports)
    )

    # Prefer sticky current WAN when it is still healthy.
    sticky_suggested = old_wan if old_wan_ok else suggested_wan
    sticky_saved = old_wan if old_wan_ok else (router.wan_interface or "")

    suggested_roles = suggest_port_roles(
        live_ports,
        suggested_wan=sticky_suggested,
        saved_wan=sticky_saved,
    )
    if old_wan_ok:
        # Force stickiness even if suggest_port_roles prefers another live hint.
        for name in list(suggested_roles.keys()):
            if _is_primary_wan_role(suggested_roles.get(name) or ""):
                if name != old_wan:
                    row = next(
                        (
                            p
                    ...
                    suggested_roles[name] = (
                        MikroTikRouter.PortRole.LAN
                        if row.get("is_bridged") or row.get("is_wireless") or row.get("running")
                        else MikroTikRouter.PortRole.UNUSED
                    )
        suggested_roles[old_wan] = MikroTikRouter.PortRole.WAN
    ...
    if _roles_maps_equal(roles, normalized) and old_wan == primary:
        label_changed = False
    else:
        label_changed = True
    ...
    message = (
        f"Labels updated automatically — Internet on {primary}, "
        "other ports set from live links."
        if label_changed
        else ""
    )
    applied_router = False
    pending_key = f"mikrotik_wan_pending:{router.pk}"
    apply_key = f"mikrotik_auto_wan_apply:{router.pk}"
    debounce_key = f"mikrotik_wan_debounce:{router.pk}"
    last_pushed = (cache.get(apply_key) or "").strip()

    # Remember a needed RouterOS sync when the labeled WAN leaves the previous port.
    if label_changed and primary and primary != old_wan:
        cache.set(pending_key, {"from": old_wan, "to": primary}, 180)

    pending = cache.get(pending_key)
    if not isinstance(pending, dict):
        pending = {}
    pending_to = (pending.get("to") or "").strip()
    pending_from = (pending.get("from") or "").strip()

    if (
        apply_on_router
        and (api_host or "").strip()
        and primary
        and pending_to == primary
        and primary != last_pushed
        and _port_has_verified_internet(primary, live_ports)
    ):
        debounce = cache.get(debounce_key)
        if not isinstance(debounce, dict):
            debounce = {}
        if (debounce.get("wan") or "").strip() == primary:
            hits = int(debounce.get("hits") or 0) + 1
        else:
            hits = 1
        cache.set(debounce_key, {"wan": primary, "hits": hits}, 180)

        if hits < 4:
            message = (
                f"Detected Internet on {primary} — confirming before switching "
                f"({hits}/4) so customers are not dropped by a flapping link."
            )
        elif last_pushed == primary:
            message = message or f"Internet already applied on {primary}."
        else:
            sync = _apply_single_wan_on_router(
                router,
                api_host,
                wan_interface=primary,
                retire_ports=[pending_from] if pending_from and pending_from != primary else [],
                live_ports=live_ports,
            )
            cache.set(apply_key, primary, 300)
            if sync.get("ok"):
                applied_router = True
                ...
                if sync.get("skipped"):
                    message = (
                        f"Internet labeled on {primary} automatically "
                        "(kept on bridge so customers stay online)."
                    )
                else:
                    message = (
                        sync.get("message")
                        or f"Internet moved to {primary} automatically — customers stay online."
                    )
            elif sync.get("error"):
                message = (
                    f"Labeled Internet on {primary}, but could not push to the MikroTik yet: "
                    f"{sync['error']}"
                )

    if not label_changed and not applied_router and not message:
        return {
            "ok": True,
            "changed": False,
            "wan": primary,
            "sticky": bool(old_wan_ok and primary == old_wan),
        }

    return {
        "ok": True,
        "changed": bool(label_changed or applied_router or message),
        "wan": primary,
        "message": message
        or (
            f"Labels updated automatically — Internet on {primary}."
        ...
        "applied_router": applied_router,
        "sticky": bool(old_wan_ok and primary == old_wan and not applied_router),
    }
    ...
    *,
    ...
    if mode == MikroTikRouter.UplinkMode.SINGLE:
        return _auto_assign_single_wan_roles(
            ...
            suggested_wan=suggested_wan,
            api_host=api_host,
            apply_on_router=apply_on_router,
        )
    if mode == MikroTikRouter.UplinkMode.BOND:
        result = _auto_assign_bond_roles(router, live_ports)
        # Always keep non-bond ports as Customers / Unused.
        members = list(result.get("members") or _bond_ports_from_roles(router))
        roles = dict(router.port_roles) if isinstance(router.port_roles, dict) else {}
        filled = _fill_non_uplink_customer_roles(
            roles, live_ports, uplink_names=members
        )
        if not _roles_maps_equal(roles, filled):
            router.port_roles = filled
            router.save(update_fields=["port_roles", "updated_at"])
            result = {
                **result,
                "changed": True,
                "message": result.get("message")
                or "Bond members and customer ports labeled automatically.",
            }
        return result
    if mode in {
        MikroTikRouter.UplinkMode.FAILOVER,
        *_weighted_share_uplink_modes(),
        ...
        primary, backups = _failover_ports_from_roles(router)
        members = [p for p in [primary, *backups] if p]
        roles = dict(router.port_roles) if isinstance(router.port_roles, dict) else {}
        filled = _fill_non_uplink_customer_roles(
            roles, live_ports, uplink_names=members
        )
        if not _roles_maps_equal(roles, filled):
            ...
            result = {
                **result,
                ...
                "changed": True,
                "message": result.get("message")
                or "ISP and customer ports labeled automatically from live links.",
            }
        return result
...
def _suggested_roles_for_uplink_mode(
    ...
    *,
    ...
    uplink_set = set(topology.get("uplinks") or [])
    customer_set = set(topology.get("customers") or [])
    unused_set = set(topology.get("unused") or [])
    primary = (topology.get("primary") or "").strip()
    ...
    if mode == MikroTikRouter.UplinkMode.SINGLE:
        for row in live_ports:
            ...
            elif name in uplink_set and name != primary:
                out[name] = MikroTikRouter.PortRole.NONE
            elif name in customer_set or row.get("is_wireless"):
                out[name] = MikroTikRouter.PortRole.LAN
            elif name in unused_set or row.get("disabled"):
                out[name] = MikroTikRouter.PortRole.UNUSED
        return out
    ...
    if mode in {
        ...
        *_weighted_share_uplink_modes(),
        ...
        uplinks = list(topology.get("uplinks") or [])
        if not uplinks:
            uplinks = _live_isp_member_ports(
                live_ports, suggested_wan=suggested_wan, saved_wan=saved_wan
            )
        if uplinks:
            out[uplinks[0]] = MikroTikRouter.PortRole.WAN
            for backup in uplinks[1:]:
                out[backup] = MikroTikRouter.PortRole.WAN_BACKUP
        for name in customer_set:
            out[name] = MikroTikRouter.PortRole.LAN
        for name in unused_set:
            out[name] = MikroTikRouter.PortRole.UNUSED
    return out
...
PORTS_ASSIGN_HINTS = {
    MikroTikRouter.UplinkMode.SINGLE: (
        "Labels update automatically from live cables — Internet follows the ISP, "
        "bridged ports stay Customers. Override only if a cable is wrong."
    ),
    MikroTikRouter.UplinkMode.BOND: (
        "Bonded internet and customer ports label themselves from live links."
    ),
    MikroTikRouter.UplinkMode.FAILOVER: (
        "ISP and customer ports label themselves — failover covers every ISP link."
    ),
    MikroTikRouter.UplinkMode.BALANCE: (
        "ISP and customer ports label themselves — failover covers every ISP link."
    ),
    MikroTikRouter.UplinkMode.SMART_BALANCE: (
        "ISP and customer ports label themselves — failover + share apply together."
    ),
}
...
def _failover_gateway_reconcile_cache_key(router_pk: int) -> str:
    return f"mikrotik_failover_gateway_reconcile:{router_pk}"


def _try_reconcile_failover_gateways(
    router: MikroTikRouter,
    api_host: str,
    *,
    primary_wan_ports: list[str],
    ...
    """
    When primary WAN still has ISP but its tagged default route is inactive,
    refresh route gateways from live DHCP/PPPoE (common after ISP renewals).
    """
    if not _is_multi_isp_mode(router.uplink_mode or ""):
        return {}
    if not uplink_live.get("ok"):
        return {}
    primary = (primary_wan_ports[0] if primary_wan_ports else "").strip()
    if not primary or not backup_wan_ports:
        return {}
    if not _failover_using_backup_route(primary, backup_wan_ports, uplink_live):
        return {}
    if not _port_has_verified_internet(primary, physical_ports):
        return {}
    primary_route = _primary_checked_route(primary, uplink_live)
    if not primary_route or primary_route.get("active"):
        return {}
    cache_key = _failover_gateway_reconcile_cache_key(router.pk)
    if cache.get(cache_key):
        return {"skipped": True, "reason": "throttled"}
    clients = uplink_live.get("failover_clients") or []
    if not clients:
        return {}
    result = reconcile_uplink_route_gateways(
        api_host,
        router.username,
        router.password or "",
...
def _try_auto_apply_smart_balance(
    ...
    *,
    ...
    ok_inet, inet_err = _guard_touch_ports_internet(
        ...
        uplink_live=uplink_live if isinstance(uplink_live, dict) else {},
        wan_share=_wan_share_snapshot_for_ports(router, live_ports or [], ordered),
    )
    if not ok_inet:
        return {"skipped": True, "reason": inet_err or "ports_need_internet"}

    blocked = _reject_behind_provider_unbridge(
        ordered, live_ports, mode_label="multi-ISP share"
    )
    if blocked:
        return {
            "skipped": True,
            "reason": blocked.get("error") or "behind_provider",
            "error": blocked.get("error"),
        }

    # Prefer tunnel management for unbridge-safe auto-apply; allow LAN when API already works.
    if not force and not _management_ready_for_uplink_apply(router, live_ports):
        return {
            "skipped": True,
            "reason": "need_tunnel_or_manual",
            "error": (
                "Multi-ISP auto-apply waits for a verified WireGuard tunnel "
                "(or use Apply manually) so MikroTik management stays online."
            ),
        }
    ...
    # Already has PCC on the router — do not tear it down and rebuild on every poll.
    # That was a major cause of repeated client drops (including monitor-only repair).
    existing_pcc = int(uplink_live.get("balance_pcc_rules") or 0)
    if not force and existing_pcc >= 2:
        cache.set(cache_key, "pcc_present", 600)
        return {
            "skipped": True,
            "reason": "pcc_already_present",
            "error": (
                "Multi-ISP sharing is already on the MikroTik. "
                "Skipping re-apply so customers stay online."
            ),
        }

    # Reserve cooldown before apply so concurrent polls cannot stampede.
    cache.set(cache_key, "pending", 120)
    weights = _suggest_balance_weights(
        ordered,
        [],
    ...
    # Never block the ports live poll on a long RouterOS push — schedule background.
    if active_uplink_apply_job(router.pk):
        return {"skipped": True, "reason": "job_already_running"}

    router_pk = router.pk
    org_pk = getattr(router, "organization_id", None) or getattr(
        getattr(router, "organization", None), "pk", None
    )
    balance_ports = list(ordered)
    balance_weights = dict(weights)
    balance_live = list(live_ports or [])
    ...
    def _auto_poll_job(
        ...
        def _progress(message: str, phase: str = "") -> None:
            touch_job(
                ...
                message=message or "Auto-applying multi-ISP…",
        ...
        if job_result.get("ok"):
            cache.set(_smart_balance_auto_cache_key(pk), "applied", 900)
            ordered_ports = job_result.get("ports") or ports
            live.uplink_mode = MikroTikRouter.UplinkMode.SMART_BALANCE
            ...
            return {
                ...
                "ports": ordered_ports,
                "weights": live.uplink_weights,
            }
        cache.set(
            _smart_balance_auto_cache_key(pk),
            (job_result.get("error") or "failed")[:200],
            180,
        )
        return job_result

    set_job(router.pk, "uplink_smart_balance", "pending")
    schedule_mikrotik_job(
        _auto_poll_job,
        name=f"uplink-smart-poll-{router.pk}",
        router_id=router.pk,
        job_type="uplink_smart_balance",
    )
    return {
        "ok": True,
        "scheduled": True,
        "auto_applied": False,
        "message": (
            "Multi-ISP apply started in the background — "
            "keep the ports page open while management reconnects."
        ),
    }


def _schedule_assigned_smart_balance_apply(
    router: MikroTikRouter,
    org,
    api_host: str,
    *,
    ...
    """Prepare roles and schedule smart balance without leaving assigned-ports."""
    applying = active_uplink_apply_job(router.pk)
    if applying:
        return {
            "ok": True,
            "scheduled": True,
            "already_running": True,
            "job_type": (applying.get("job_type") or "uplink_smart_balance"),
            "message": "Smart balance apply is already running.",
        }
    ...
    if not assign.get("ok"):
        return {
            "ok": False,
            "error": assign.get("error") or "Need two live ISP links for smart balance.",
        }
    ...
    if len(ordered) < 2:
        return {
            "ok": False,
            "error": "Assign Internet and Shared ISP on two WAN ports, then retry.",
        }

    weights = dict(assign.get("weights") or router.uplink_weights or {})
    uplink_live, wan_share = _apply_internet_guard_context(
        router, api_host, live_ports, ordered
    )
    ok_inet, inet_err = _guard_touch_ports_internet(
        ordered,
        live_ports,
    ...
    if not ok_inet:
        return {"ok": False, "error": inet_err}
    ...
    if not force and not _management_ready_for_uplink_apply(router, live_ports):
        return {
            "ok": False,
            "error": _uplink_tunnel_ready_message(router),
        }

    cache.delete(_smart_balance_auto_cache_key(router.pk))
    cache.delete(_assigned_balance_auto_cache_key(router.pk))
    ...
    if result.get("ok") and result.get("scheduled"):
        result.setdefault(
            "message",
            "Smart balance apply started — keep this page open while it completes.",
        )
        result["job_type"] = "uplink_smart_balance"
    elif result.get("skipped"):
        return {
            "ok": False,
            "error": result.get("error") or result.get("reason") or "Could not start apply.",
        }
    return result
...
def _primary_route_distance(primary: str, uplink_live: dict) -> str:
    for client in uplink_live.get("failover_clients") or []:
        if client.get("disabled"):
            continue
        if (client.get("interface") or "").strip() == (primary or "").strip():
            return str(client.get("distance") or "1")
    return "1"


def _primary_checked_route(primary: str, uplink_live: dict) -> dict | None:
    """Tagged default route for the primary WAN distance, if present."""
    primary = (primary or "").strip()
    if not primary:
        return None
    dist = _primary_route_distance(primary, uplink_live)
    for route in uplink_live.get("checked_routes") or []:
        if str(route.get("distance") or "1") == dist:
            return route
    return None


def _active_failover_route(uplink_live: dict) -> dict | None:
    checked = uplink_live.get("checked_routes") or []
    return next(
        (row for row in checked if row.get("active") and not row.get("disabled")),
        None,
...
def _failover_using_backup_route(
    primary: str,
    backup_wan_ports: list[str],
    uplink_live: dict,
) -> bool:
    """True when the live default route is on a backup/shared port, not the primary."""
    primary = (primary or "").strip()
    backups = {str(p).strip() for p in (backup_wan_ports or []) if str(p).strip()}
    if not primary or not backups:
        return False
    active = _active_failover_route(uplink_live)
    if not active:
        return False
    active_dist = str(active.get("distance") or "1")
    primary_dist = _primary_route_distance(primary, uplink_live)
    if active_dist == primary_dist:
        return False
    clients = uplink_live.get("failover_clients") or []
    for client in clients:
        if client.get("disabled"):
            continue
        if str(client.get("distance") or "1") != active_dist:
            continue
        iface = (client.get("interface") or "").strip()
        if iface in backups:
            return True
...
def _analyze_wan_failover_reason(
    port_name: str,
    row: dict,
    physical_ports: list[dict],
    *,
    uplink_live: dict | None = None,
    backup_wan_ports: list[str] | None = None,
    smart_balance_status: dict | None = None,
) -> dict[str, str]:
    """Operator-facing reason + recovery text for primary WAN failover."""
    name = (port_name or "").strip()
    if not name:
        return {
            "code": "missing",
            "reason": "Primary internet port is unavailable",
            "recovery": "when a primary port is assigned",
        }
    if row.get("disabled"):
        return {
            "code": "disabled",
            "reason": f"{name} is disabled",
            "recovery": f"when {name} is enabled again",
        }
    if not row.get("running"):
        return {
            "code": "link_down",
            "reason": f"{name} has no cable link",
            "recovery": f"when {name} link comes back",
        }
    if not _port_has_verified_internet(name, physical_ports):
        meta = _uplink_candidate_status(row)
        status = (meta.get("status") or "").strip()
        if status == "waiting_dhcp":
            return {
                "code": "waiting_isp",
                "reason": f"{name} is waiting for ISP (DHCP)",
                "recovery": f"when {name} gets an ISP lease",
            }
        if status == "waiting_pppoe":
            return {
                "code": "waiting_isp",
                "reason": f"{name} is waiting for ISP (PPPoE)",
                "recovery": f"when {name} connects to the ISP",
            }
        if status == "link_up":
            return {
                "code": "no_isp",
                "reason": f"{name} is linked but ISP is not verified yet",
                "recovery": f"when {name} shows ISP online",
            }
        return {
            "code": "no_isp",
            "reason": f"{name} has no working ISP",
            "recovery": f"when {name} has working ISP",
        }

    live = uplink_live if isinstance(uplink_live, dict) else {}
    backups = list(backup_wan_ports or [])
    slow_ports = {
        str(p).strip()
        for p in (
    ...
    if name in slow_ports:
        return {
            "code": "smart_balance_slow",
            "reason": f"{name} is sidelined (slow or unstable ISP)",
            "recovery": f"when {name} quality improves",
        }

    if not _failover_using_backup_route(name, backups, live):
        return {
            "code": "unknown",
            "reason": f"{name} is not carrying traffic",
            "recovery": f"when {name} is healthy again",
        }

    primary_route = _primary_checked_route(name, live)
    if primary_route:
        if primary_route.get("disabled"):
            return {
                "code": "route_disabled",
                "reason": f"{name} default route is disabled on the router",
                "recovery": f"when {name} default route is enabled again",
            }
        gateway = (primary_route.get("gateway") or "").strip()
        if not primary_route.get("active"):
            if (primary_route.get("check_gateway") or "").strip().lower() == "ping":
                reason = f"{name} failed gateway health checks"
                if gateway:
                    reason += f" (cannot reach gateway {gateway})"
                return {
                    "code": "routing_health",
                    "reason": reason,
                    "recovery": f"when {name} passes gateway health checks again",
                }
            return {
                "code": "route_inactive",
                "reason": f"{name} default route is not active",
                "recovery": f"when {name} default route becomes active again",
            }
...
def _build_failover_on_backup_alert(
    *,
    uplink_mode: str,
    primary_wan_ports: list[str],
    backup_wan_ports: list[str],
    ...
    """Human-readable alert when multi-ISP traffic is on a backup/shared port."""
    primary = (primary_wan_ports[0] if primary_wan_ports else "").strip()
    primary_row = by_name.get(primary) or {}
    ...
    if active and clients:
        ...
        for c in clients:
            ...
            if iface in backup_wan_ports and iface not in active_backups:
                active_backups.append(iface)
    if not active_backups:
        active_backups = list(backup_up)

    using = ", ".join(active_backups[:2]) if active_backups else "backup link"
    analysis = (
        _analyze_wan_failover_reason(
            primary,
            primary_row,
            physical_ports,
            uplink_live=uplink_live,
    ...
    primary_label = _friendly_role_label(MikroTikRouter.PortRole.WAN, mode=uplink_mode)
    backup_label = _friendly_role_label(
        MikroTikRouter.PortRole.WAN_BACKUP, mode=uplink_mode
    )
    mode = (uplink_mode or "").strip()

    if _is_weighted_share_mode(mode):
        title = f"Internet flowing through {using}"
        if primary:
            message = (
                f"{reason}. Customers stay online — traffic returns to "
                f"{primary} ({primary_label}) {recovery}."
            )
        else:
            message = f"{reason}. Customers stay online on {using}."
    elif mode == MikroTikRouter.UplinkMode.FAILOVER:
        title = f"Using {using} ({backup_label.lower()})"
        if primary:
            message = (
                f"{reason}. Traffic returns to {primary} ({primary_label}) "
                f"{recovery}."
            )
        else:
            message = f"{reason}. Using {using} until the primary link recovers."
    else:
        title = f"Internet on {using}"
        message = (
            f"{reason}. Traffic returns to {primary} {recovery}."
            if primary
            else f"{reason}. Using {using}."
        )

    standby_ready = [n for n in backup_up if n not in active_backups]
    if standby_ready:
        ready = ", ".join(standby_ready[:2])
        message += f" {ready} stay ready if needed."
    ...
    return {
        ...
        "title": title,
        "message": message,
        "reason_code": analysis.get("code") or "",
        "active_ports": active_backups,
...
def _build_uplink_health_alerts(
    *,
    ...
    if mode == MikroTikRouter.UplinkMode.SINGLE and primary_wan_ports:
        primary = (primary_wan_ports[0] or "").strip()
        row = by_name.get(primary) or {}
        if row.get("disabled") or not row.get("running"):
            backup_ready = [
                name
                for name, prow in by_name.items()
                if name != primary
                and _port_has_verified_internet(name, physical_ports)
            ]
            msg = f"Internet port {primary} has no link."
            if backup_ready:
                msg += (
                    f" Failover to {', '.join(backup_ready[:2])} will apply automatically "
                    "once confirmed (about 12 seconds)."
                )
            alerts.append(
                {
                    "level": "warn",
                    "code": "primary_wan_down",
                    "message": msg,
                }
            )
        elif not _port_has_verified_internet(primary, physical_ports):
            backup_ready = [
                name
                for name, prow in by_name.items()
                if name != primary
            ...
            msg = f"Internet port {primary} lost ISP connectivity."
            if backup_ready:
                msg += (
                    f" Switching to {', '.join(backup_ready[:2])} automatically "
                    "once the new link is stable."
                )
            else:
                msg += " Waiting for another port to get ISP online."
            alerts.append(
                {
                    "level": "warn",
                    "code": "primary_wan_no_isp",
                    "message": msg,
                }
            )
        extra_links = []
        primary_name = (primary_wan_ports[0] if primary_wan_ports else "").strip()
        port_roles = {
            (p.get("name") or "").strip(): (p.get("role") or "").strip()
            for p in physical_ports
            if (p.get("name") or "").strip()
        }
        for name, prow in by_name.items():
            if not _is_backup_isp_candidate(
                name,
                prow,
                roles=port_roles,
                primary=primary_name,
                known_backups=set(backup_wan_ports or []),
                live_ports=physical_ports,
            ):
                continue
            extra_links.append(name)
        if extra_links and primary_name and _port_has_verified_internet(
            primary_name, physical_ports
        ):
            ports_label = ", ".join(extra_links[:3])
            alerts.append(
                {
                    "level": "info",
                    "code": "multi_uplink_available",
                    "message": (
                        f"Second ISP detected on {ports_label}. "
                        "Set up multiple ISPs to share traffic and fail over automatically."
                    ),
                    "action": "set_multi_isp",
                    "action_label": "Set up multiple ISPs",
                    "isp_ports": extra_links[:3],
                }
            )
    ...
    # Failover health applies to every multi-ISP mode (pure failover + share modes).
    if _is_multi_isp_mode(mode) and (primary_wan_ports or backup_wan_ports):
        primary_down = []
        ...
        if active and clients:
            ...
            on_backup = True
        live_mode = (uplink_live.get("mode") or "").strip()
        if (
            not on_backup
            and _is_weighted_share_mode(mode)
            and live_mode in _weighted_share_uplink_modes()
            and primary_down
            and backup_up
        ):
            on_backup = True
        ...
        if (
            ...
            and primary_name
            and backup_up
            and _failover_using_backup_route(
                primary_name, backup_wan_ports, uplink_live
            )
        ):
            on_backup = True
        if on_backup and primary_name:
            primary_route = _primary_checked_route(primary_name, uplink_live)
            primary_row = by_name.get(primary_name) or {}
            if (
                primary_route
                and primary_route.get("active")
                and not primary_route.get("disabled")
                and primary_row.get("running")
                and not primary_row.get("disabled")
                and _port_has_verified_internet(primary_name, physical_ports)
                and not _failover_using_backup_route(
                    primary_name, backup_wan_ports, uplink_live
                )
            ):
                on_backup = False
        if on_backup:
            alerts.append(
                _build_failover_on_backup_alert(
                    uplink_mode=mode,
                    primary_wan_ports=primary_wan_ports,
                    backup_wan_ports=backup_wan_ports,
                    backup_up=backup_up,
        ...
        for name in standby_down:
            alerts.append(
                {
                    "level": "info",
                    "code": "backup_down",
                    "message": (
                        f"Standby {name} has no link — failover has one fewer backup."
                    ),
                }
            )
            ...
            alerts.append(
                {
                    ...
                    "message": (
                        ...
                        "failover keeps traffic on surviving links."
                    ),
                }
            )
        if no_inet and not balance_router_applied:
            alerts.append(
                {
                    "level": "warn",
...
def _suppress_redundant_health_alerts(
    alerts: list[dict],
    *,
    backup_uplink_prompt: dict | None,
    uplink_prompt: dict | None,
    recommendation: dict | None,
) -> list[dict]:
    """Drop noisy or duplicate health banners; keep one clear operator message."""
    items = list(alerts or [])
    codes = {(a or {}).get("code") for a in items if (a or {}).get("code")}
    rec = recommendation if isinstance(recommendation, dict) else {}
    has_action = bool(
        backup_uplink_prompt
        or uplink_prompt
        or rec.get("can_accept")
        or rec.get("ready_to_apply")
    )
    drop: set[str] = set()
    if has_action:
        drop.add("balance_not_applied")
    if backup_uplink_prompt:
        drop.update({"balance_share_drift", "smart_balance_monitor_off"})
    if "failover_on_backup" in codes:
        drop.update({"balance_share_drift", "smart_balance_monitor_off"})
    # Monitor reinstalls itself — not worth a top-of-page banner.
    drop.add("smart_balance_monitor_off")
    if not drop:
        return items
    return [a for a in items if (a or {}).get("code") not in drop]
...
def _is_backup_isp_candidate(
    name: str,
    row: dict,
    *,
    roles: dict,
    primary: str,
    known_backups: set[str],
    live_ports: list[dict],
) -> bool:
    """True when a live port plausibly carries a second ISP, not a customer link."""
    name = (name or "").strip()
    if not name or name == primary or name in known_backups:
        return False
    role = (roles.get(name) or "").strip().lower()
    if role in {
        MikroTikRouter.PortRole.LAN,
        MikroTikRouter.PortRole.UNUSED,
        MikroTikRouter.PortRole.BOND,
    ...
    if not row.get("running"):
        return False
    topology = _infer_port_topology(live_ports, saved_wan=primary)
    if name in set(topology.get("customers") or []):
        return False
    if _port_has_isp_signals(row) or _port_has_verified_internet(name, live_ports):
        return True
    return False
...
def _build_backup_uplink_prompt(
    ...
    *,
    ...
    primary_ports, backup_ports = _failover_ports_from_roles(router)
    if primary_ports and backup_ports:
        return None
    ...
    return {
        ...
        "kind": "add_backup",
        "message": (
            f"{candidate} is linked ({kind_label}). "
            + (
                "Mark as Shared ISP — smart balance auto-applies when links are up."
                if mode == MikroTikRouter.UplinkMode.SMART_BALANCE
                else (
                    f"Mark as Shared ISP, then apply {mode_apply}."
                    if _is_weighted_share_mode(mode)
                    else f"Mark as Backup internet, then apply {mode_apply}."
                )
            )
        ),
    }
...
def suggest_port_roles(
    ...
    *,
    ...
    - Internet + optional second uplink from topology (ether1/ether2 typical)
    - bridge members after uplink block → Customers (LAN)
    - disabled / quiet unused ports → Unused
    """
    topology = _infer_port_topology(
        ports, suggested_wan=suggested_wan, saved_wan=saved_wan
    )
    primary = (topology.get("primary") or "").strip()
    uplinks = list(topology.get("uplinks") or [])
    uplink_set = set(uplinks)
    customer_set = set(topology.get("customers") or [])
    unused_set = set(topology.get("unused") or [])
...
@client_workspace_required
@require_GET
def mikrotik_recovery_scripts(request, router_id: int):
    """Winbox copy-paste scripts for on-site MikroTik emergency recovery."""
    org = resolve_organization(request.user, request)
    if not org:
        messages.error(request, "No organization is linked to this workspace.")
        return redirect("core:mikrotik")
    ...
    ctx = client_page_context(
        request,
        active_nav="mikrotik_detail",
        sidebar_active="recovery_scripts",
        page_title=f"{router.name} — Winbox recovery",
        page_kicker="Emergency",
        page_subtitle=(
            "Copy-paste RouterOS commands for on-site recovery when billing "
            "cannot reach this MikroTik or clients cannot browse."
        ),
        router=router,
        is_suspended=is_suspended,
        recovery_sections=build_mikrotik_recovery_script_sections(router),
        mikrotik_quicknav_set="overview",
        mikrotik_quicknav_active="recovery_scripts",
    )
    return render(
        request,
        "core/mikrotik_recovery_scripts.html",
        apply_mikrotik_detail_sidebar(ctx, router, detail_nav=detail_nav),
    )
...
def mikrotik_ports(request, router_id: int):
    ...
    if request.method == "POST":
        ...
        if action == "accept_uplink_recommendation":
            listed = list_mikrotik_ports(
                api_host,
                router.username,
                router.password or "",
            ...
            if not result.get("ok"):
                messages.error(
                    request,
                    result.get("error") or "Could not apply the recommended links.",
                )
                return redirect("core:mikrotik_ports", router_id=router.pk)

            mode = (router.uplink_mode or MikroTikRouter.UplinkMode.SINGLE).strip()
            if mode == MikroTikRouter.UplinkMode.SINGLE:
                port_name = (result.get("wan") or "").strip()
                old_wan = (result.get("old_wan") or "").strip()
                if port_name:
                    retire = [old_wan] if old_wan and old_wan != port_name else []
                    _store_wan_switch_rollback(
                        router, old_wan=old_wan, new_wan=port_name
                    )
                    sync = _apply_single_wan_on_router(
                        router,
                        api_host,
                    ...
                    if not sync.get("ok"):
                        messages.warning(
                            ...
                            sync.get("error")
                            or (
                                f"Saved {port_name} as Internet, but could not "
                        ...
                        messages.success(
                            ...
                            or "Recommended Internet port applied.",
                        )
                else:
                    messages.success(
                        request,
                        result.get("message") or "Recommended Internet port saved.",
                    )
            elif mode == MikroTikRouter.UplinkMode.SMART_BALANCE:
                router.refresh_from_db()
                primary, backups = _failover_ports_from_roles(router)
                members = [p for p in [primary, *backups] if p]
                if len(members) >= 2:
                    auto = _try_auto_apply_smart_balance(
                        router,
                        api_host,
                    ...
                    if auto.get("ok"):
                        messages.success(
                            ...
                            auto.get("message")
                            or result.get("message")
                            or "Recommended multi-ISP links applied.",
                        )
                    else:
                        messages.success(
                            request,
                            result.get("message")
                            or "Recommended ISP links labeled — apply when ready.",
                        )
                else:
                    messages.success(
                        request,
                        result.get("message") or "Recommended ISP links labeled.",
                    )
            else:
                messages.success(
                    request,
                    result.get("message") or "Recommended links labeled.",
                )

            _record_ports_audit(
                request,
                action="ports_accept_uplink_recommendation",
                router=router,
                detail={
                    "mode": mode,
                    "primary": result.get("primary") or result.get("wan") or "",
        ...
        if action == "set_uplink_goal":
            raw_goal = (request.POST.get("goal") or "").strip().lower()
            goal = _normalize_uplink_goal(raw_goal)
            if not goal:
            ...
            # Entering multi-ISP / bond: hot-apply when ports are already ready so
            # the setup works without a manual Apply click (no client disconnect).
            auto_apply_msg = ""
            if goal == MikroTikRouter.UplinkMode.SMART_BALANCE and live_ports:
                primary_ports, backup_ports = _failover_ports_from_roles(router)
                members = [p for p in [primary_ports, *backup_ports] if p]
                ready, _hint = _balance_apply_readiness(
                    [primary_ports] if primary_ports else [],
                    backup_ports,
                    live_ports,
                )
                if ready and len(members) >= 2:
                    blocked = _reject_behind_provider_unbridge(
                        members, live_ports, mode_label="multi-ISP share"
                    )
                    if blocked:
                        messages.info(
                            request,
                        ...
                        router_pk = router.pk
                        org_pk = org.pk
                        goal_members = list(members)
                        goal_weights = (
                            dict(router.uplink_weights)
                            if isinstance(router.uplink_weights, dict)
                            else {}
                        )
                        goal_live = list(live_ports)

                        def _goal_multi_job(
                            ports=goal_members,
                            port_weights=goal_weights,
                            pk=router_pk,
                            organization_pk=org_pk,
                            ports_snapshot=goal_live,
                        ):
                            live = MikroTikRouter.objects.get(pk=pk)
                            host = _router_api_host(live)
                            ...
                            def _progress(message: str, phase: str = "") -> None:
                                touch_job(
                                    pk,
                                    "uplink_smart_balance",
                                    message=message or "Applying multi-ISP…",
                                    phase=phase or "configure",
                                )
                            ...
                            if job_result.get("ok"):
                                live.uplink_unbridged = job_result.get("unbridged") or []
                                live.wan_interface = (
                                    job_result.get("wan_interface")
                                    or (ports[0] if ports else live.wan_interface)
                                )
                                live.uplink_ports = list(
                                    job_result.get("ports") or ports
                                )
                                if isinstance(job_result.get("weights"), dict):
                                    live.uplink_weights = job_result["weights"]
                                live.port_roles = _sync_roles_for_uplink(
                                    live,
                                    mode=MikroTikRouter.UplinkMode.SMART_BALANCE,
                                    ports=live.uplink_ports,
                                )
                                live.save(
                                    update_fields=[
                                        "uplink_unbridged",
                                ...
                                _invalidate_mikrotik_router_caches(
                                    organization_pk, pk
                                )
                                cache.set(
                                    _smart_balance_auto_cache_key(pk), "applied", 900
                                )
                            return job_result
                        ...
                        schedule_mikrotik_job(
                            _goal_multi_job,
                            name=f"uplink-smart-goal-{router.pk}",
                            router_id=router.pk,
                            job_type="uplink_smart_balance",
                        )
                        auto_apply_msg = (
                            "Multi-ISP is applying in the background — "
                            "keep this page open while management reconnects."
                        )
            elif goal == MikroTikRouter.UplinkMode.BOND and live_ports:
                members = _bond_ports_from_roles(router)
                bond_ready, _hint = _bond_apply_readiness(members, live_ports)
                if bond_ready and len(members) >= 2:
                    blocked = _reject_behind_provider_unbridge(
                    ...
                    if blocked:
                        ...
                        bond_mode = router.bond_mode or "balance-xor"
                        router_pk = router.pk
                        org_pk = org.pk
                        bond_members = list(members)
                        bond_live = list(live_ports)
                        ...
                        def _goal_bond_job(
                            ...
                            pk=router_pk,
                            organization_pk=org_pk,
                            ports_snapshot=bond_live,
                        ):
                            live = MikroTikRouter.objects.get(pk=pk)
                            host = _router_api_host(live)

                            def _bond_progress(message: str, phase: str = "") -> None:
                                touch_job(
                                    pk,
                                    "uplink_bond",
                                    message=message or "Applying bonding…",
                                    phase=phase or "configure",
                                )
                            ...
                            job_result = apply_mikrotik_uplink_bond(
                                ...
                                api_hosts=_uplink_api_hosts_for_router(live),
                                live_ports=ports_snapshot,
                                progress=_bond_progress,
                            )
                            job_result = _finalize_uplink_apply_result(live, job_result)
                            if job_result.get("ok"):
                                applied_members = job_result.get("members") or ports
                                live.uplink_unbridged = job_result.get("unbridged") or []
                                live.bond_interface = (
                                    job_result.get("bond_name") or bname
                                )
                                live.bond_mode = job_result.get("bond_mode") or bmode
                                live.wan_interface = (
                                    job_result.get("wan_interface")
                                    or live.bond_interface
                                )
                                live.uplink_ports = list(applied_members)
                                live.port_roles = _sync_roles_for_uplink(
                                    live,
                                    mode=MikroTikRouter.UplinkMode.BOND,
                                    ports=applied_members,
                                )
                                live.save(
                                    update_fields=[
                                        "uplink_unbridged",
                        ...
                        schedule_mikrotik_job(
                            _goal_bond_job,
                            name=f"uplink-bond-goal-{router.pk}",
                            router_id=router.pk,
                            job_type="uplink_bond",
                        )
                        auto_apply_msg = (
                            "Bonding is applying in the background — "
                            "keep this page open while management reconnects."
                        )
            ...
            label = _ui_uplink_goal_label(goal)
            if auto_apply_msg:
                messages.success(
                    request,
                    f"Internet setup changed to {label}. {auto_apply_msg}",
                )
            else:
        ...
        if action == "set_port_role":
            ...
            if not _role_allowed_for_uplink_mode(role, uplink_mode):
                ...
                messages.error(
                    ...
                    "Change the internet setup to Multiple ISPs first.",
                )
                return redirect("core:mikrotik_ports", router_id=router.pk)
        ...
        if action == "apply_bond":
            ...
            # Always background: unbridging flaps management; keep the ports UI usable.
            set_job(router.pk, "uplink_bond", "pending")
            schedule_mikrotik_job(
                _apply_bond_job,
            ...
            messages.success(
                ...
                "A brief disconnect is normal — keep this page open.",
            )
            return _redirect_with_mikrotik_job(
        ...
        if action == "apply_failover":
            ...
            uplink_live, wan_share = _apply_internet_guard_context(
                router, api_host, live_ports, touch
            )
            ok_inet, inet_err = _guard_touch_ports_internet(
                touch,
                live_ports,
            ...
            def _apply_failover_job(
                ...
                def _progress(message: str, phase: str = "") -> None:
                    touch_job(
                        pk,
                        "uplink_failover",
                        message=message or "Applying failover…",
                        phase=phase or "configure",
                    )
            ...
            _record_ports_audit(
                ...
                router=router,
                detail={
                    "ports": [failover_primary, *failover_backups],
                    "background": True,
                },
            )
            messages.success(
        ...
        if action == "auto_setup_bond":
            ...
            schedule_mikrotik_job(
                _auto_bond_job,
                name=f"uplink-bond-auto-{router.pk}",
                router_id=router.pk,
                job_type="uplink_bond",
            )
            _record_ports_audit(
            ...
            messages.success(
                request,
                "Detect & apply bonding started in the background. "
                "Keep this page open while management reconnects.",
            )
            return _redirect_with_mikrotik_job(
                request, "core:mikrotik_ports", router.pk, "uplink_bond"
        ...
        if action == "auto_setup_smart_balance":
            ...
            schedule_mikrotik_job(
                _auto_smart_job,
                name=f"uplink-smart-auto-{router.pk}",
                router_id=router.pk,
                job_type="uplink_smart_balance",
            )
            _record_ports_audit(
            ...
            messages.success(
                request,
                "Detect & apply multi-ISP started in the background. "
                "Keep this page open while management reconnects.",
            )
            return _redirect_with_mikrotik_job(
                request, "core:mikrotik_ports", router.pk, "uplink_smart_balance"
            )
        ...
        if action in {"apply_balance", "apply_smart_balance"}:
            ...
            def _apply_balance_job(
                ...
                def _progress(message: str, phase: str = "") -> None:
                    touch_job(
                        ...
                        job_name,
                        message=message
                        or f"Applying {'smart balance' if smart else 'load balance'}…",
                        phase=phase or "configure",
                    )
            ...
            _record_ports_audit(
                request,
                action=(
                    "ports_apply_smart_balance"
                    if smart_balance
                    else "ports_apply_balance"
                ),
                router=router,
                detail={
                    "ports": balance_ports,
            ...
            messages.success(
                request,
                f"Applying {apply_label} on the MikroTik in the background. "
                "A brief disconnect is normal — keep this page open.",
            )
            return _redirect_with_mikrotik_job(
    ...
    ctx = client_page_context(
        ...
        uplink_mode=uplink_mode,
        uplink_mode_label=_ui_uplink_goal_label(uplink_mode),
...
def _ports_from_stored_roles(router: MikroTikRouter) -> list[dict]:
    """Minimal port rows from saved roles so the UI can keep rendering mid-apply."""
    roles = router.port_roles if isinstance(router.port_roles, dict) else {}
    uplink_ports = [
        str(p).strip() for p in (router.uplink_ports or []) if str(p).strip()
    ]
    names = list(dict.fromkeys([*(roles.keys()), *uplink_ports]))
    if not names:
        names = ["ether1", "ether2", "ether3", "ether4", "ether5"]
    out: list[dict] = []
    for name in names:
        role = (roles.get(name) or "").strip().lower() or MikroTikRouter.PortRole.NONE
        is_bond_member = (
            role == MikroTikRouter.PortRole.BOND or name in uplink_ports
        ) and (router.uplink_mode or "") == MikroTikRouter.UplinkMode.BOND
        out.append(
            {
                "name": name,
                "type": "ether" if not str(name).startswith("wlan") else "wlan",
                "running": True,
                "disabled": False,
                "is_bridged": role == MikroTikRouter.PortRole.LAN,
                "is_wireless": str(name).startswith("wlan"),
                "role": role,
                "suggested_role": role,
                "uplink_kind": "dhcp" if is_bond_member else "",
                "uplink_active": False,
                "internet_verified": False,
                "internet_level": "warn",
                "internet_hint": "Applying uplink — live status paused",
                "auto_managed": True,
            }
        )
    return out
...
def _ports_live_payload_while_applying(
    router: MikroTikRouter,
    *,
    applying: dict,
    listed_error: str = "",
    wan_rollback: dict | None = None,
) -> dict:
    """Soft-success payload while bond/failover apply flaps the management API."""
    uplink_mode = router.uplink_mode or MikroTikRouter.UplinkMode.SINGLE
    ports = _ports_from_stored_roles(router)
    physical_ports = [p for p in ports if not str(p.get("name") or "").startswith("bond")]
    job_type = (applying.get("job_type") or "").strip()
    job_label = {
        "uplink_bond": "bonding",
        "uplink_failover": "failover",
        "uplink_balance": "load balance",
        "uplink_smart_balance": "multi-ISP",
        "clean_uplink": "clean uplink",
    }.get(job_type, "uplink")
    setup_message = (
        f"{job_label.capitalize()} is applying on the MikroTik in the background. "
        "A brief API timeout is normal while ISP cables leave the LAN bridge — "
        "keep this page open; you can continue when the apply finishes."
    )
    bond_member_ports = [
    ...
    payload = {
        "ok": True,
        "applying_uplink": True,
        "ports_temporarily_unreachable": True,
        "uplink_apply_job": {
            "job_type": job_type,
            "status": applying.get("status") or "running",
            "message": applying.get("message") or "",
            "phase": applying.get("phase") or "",
        },
        "message": setup_message,
        "ports": ports,
        "physical_ports": physical_ports,
        "suggested_wan": router.wan_interface or "",
        "auto_assigned": False,
        "auto_assigned_message": setup_message,
        "role_sync_message": setup_message,
        "uplink_live": {},
        ...
        "wan_switch_risks": {},
        "uplink_health_alerts": [
            {
                "level": "info",
                "code": "uplink_applying",
                "message": setup_message,
            }
        ],
        "uplink_setup_status": {
            ...
            "ok": True,
            "level": "info",
            "title": f"Applying {job_label} — you can wait here",
            "phase": "applying",
            "message": setup_message,
            "problems": [],
        },
        "can_apply_bond": False,
        "bond_apply_hint": setup_message,
        "can_auto_setup_bond": False,
        "can_apply_failover": False,
        "can_apply_balance": False,
        "can_apply_smart_balance": False,
        "can_apply_multi": False,
        "balance_apply_hint": setup_message,
        "balance_router_applied": False,
        "smart_balance_applied": False,
        "smart_balance_health": {},
        "uplink_mode": uplink_mode,
        "ui_uplink_goal": _ui_uplink_goal(uplink_mode),
        "uplink_mode_label": _ui_uplink_goal_label(uplink_mode),
        "uplink_weights": (
            dict(router.uplink_weights)
            if isinstance(router.uplink_weights, dict)
        ...
        "uplink_recommendation": {
            "applies": True,
            "phase": "applying",
            "level": "info",
            "title": f"Applying {job_label}",
            "message": setup_message,
            "can_accept": False,
            "candidates": [],
            "recommended_ports": list(router.uplink_ports or []),
            "waiting_ports": [],
            "accept_label": "Use recommendation",
        },
        "uplink_apply_risks": {},
        "listed_error": listed_error,
    }
    if wan_rollback:
...
def _ports_live_payload(
    ...
    if not listed.get("ok"):
        applying = active_uplink_apply_job(router.pk)
        if applying:
            # Bond/failover unbridge flaps API briefly — keep the operator on the
            # ports page with stored labels instead of a hard timeout wall.
            return _ports_live_payload_while_applying(
                router,
                applying=applying,
                listed_error=listed.get("error") or "",
    ...
    for row in live_ports:
        ...
        show_internet_status = _port_shows_internet_status(role or "")
        if (role or "").strip() == MikroTikRouter.PortRole.BOND:
            # Bond slaves are link-only — ISP is on the bond interface after apply.
            internet_hint = ""
            show_internet_status = False
        elif _is_bond_port_row(row):
            show_internet_status = True
        elif not show_internet_status:
            internet_hint = ""
        elif (
            (role or "").strip() == MikroTikRouter.PortRole.WAN_BACKUP
            and row.get("is_bridged")
    ...
    bond_router_applied = _bond_router_applied(
        uplink_mode,
        uplink_live,
        bond_member_ports=bond_member_ports,
        bond_interface=(router.bond_interface or "").strip(),
    ...
    if _is_multi_isp_mode(uplink_mode) and uplink_live.get("ok"):
        failover_active_port = _failover_active_wan_port(
            uplink_live,
            primary_wan_ports=primary_wan_ports,
            backup_wan_ports=backup_wan_ports,
        )
    for port in ports:
        port.setdefault("failover_carrying", False)
        port.setdefault("failover_primary_stood_down", False)
        port.setdefault("sharing_traffic", False)
    lan_ports = [
        p["name"] for p in physical_ports if p.get("role") == MikroTikRouter.PortRole.LAN
    ]
    unused_ports = [
    ...
    bond_ready, bond_apply_hint = _bond_apply_readiness(
        bond_member_ports,
        physical_ports,
        bond_applied=bond_router_applied,
    )
    ...
    uplink_recommendation = _build_uplink_recommendation(
        router,
        physical_ports,
        suggested_wan=suggested_wan,
        dual_wan_ready=dual_wan_ready,
        bond_ready=bond_ready,
        balance_ready=balance_ready,
        balance_router_applied=balance_router_applied,
        smart_balance_applied=smart_balance_applied,
        bond_router_applied=bond_router_applied,
    )
    uplink_health_alerts = _suppress_redundant_health_alerts(
        uplink_health_alerts,
        backup_uplink_prompt=backup_uplink_prompt,
        uplink_prompt=uplink_prompt,
        recommendation=uplink_recommendation,
    )
    uplink_setup_status = _build_uplink_setup_status(
        uplink_mode=uplink_mode,
        primary_wan_ports=primary_wan_ports,
        backup_wan_ports=backup_wan_ports,
        bond_member_ports=bond_member_ports,
        ...
        smart_balance_applied=smart_balance_applied,
        uplink_live=uplink_live if uplink_live.get("ok") else {},
        health_alerts=uplink_health_alerts,
        recommendation=uplink_recommendation,
        bond_router_applied=bond_router_applied,
        wan_share=wan_share if wan_share.get("ok") else {},
    )
    ...
    # Assigned-ports page reads usage; skip on the ports poll to keep it light.
    router_analysis: dict = {}
    balance_insights: dict = {}
    balance_auto: dict = {}
    ...
    if include_client_analysis and listed.get("ok"):
        ...
        balance_insights = _build_client_balance_insights(
            router,
            router_analysis,
            uplink_mode=uplink_mode,
            physical_ports=physical_ports,
            ...
            if balance_auto.get("ok"):
                try:
                    uplink_live = read_mikrotik_uplink_multi(
                        ...
                        router.password or "",
                        timeout=8.0,
                        member_ports=list(router.uplink_ports or member_ports),
                    )
                    smart_balance_status = (
                        uplink_live.get("smart_balance_status")
                        if isinstance(uplink_live.get("smart_balance_status"), dict)
                    ...
                    balance_router_applied = (
                        _is_weighted_share_mode(uplink_mode)
                        and bool(uplink_live.get("ok"))
                        and (uplink_live.get("mode") or "").strip()
                        in _weighted_share_uplink_modes()
                    )
                    smart_balance_applied = bool(balance_router_applied) and (
                        uplink_mode != MikroTikRouter.UplinkMode.SMART_BALANCE
                        or bool(
    ...
    return {
        ...
        "uplink_setup_status": uplink_setup_status,
        "can_apply_bond": (
        ...
        "can_apply_smart_balance": (
            _is_multi_isp_mode(uplink_mode) and balance_ready
        ),
        "can_apply_multi": (
            _is_multi_isp_mode(uplink_mode) and (dual_wan_ready or balance_ready)
        ),
        ...
        "uplink_prompt": uplink_prompt,
        "backup_uplink_prompt": backup_uplink_prompt,
        "uplink_recommendation": uplink_recommendation,
        "uplink_apply_risks": uplink_apply_risks,
        "tunnel_management": tunnel_management,
        "router_analysis": router_analysis,
        "balance_insights": balance_insights,
        "balance_auto": balance_auto,
        "client_rebalance": client_rebalance,
...
@client_workspace_required
@require_GET
def mikrotik_assigned_ports_live(request, router_id: int):
    """JSON live client + port analytics for the assigned-ports page."""
    org = resolve_organization(request.user, request)
    if not org:
        return JsonResponse({"ok": False, "error": "No organization."}, status=400)
    ...
    if router.account_status == MikroTikRouter.AccountStatus.SUSPENDED:
        return JsonResponse(
            {
                "ok": False,
                "error": "Activate this MikroTik account to view assigned ports.",
                "suspended": True,
            }
        )
    ...
    try:
        payload = _ports_live_payload(router, include_client_analysis=True)
    except Exception as exc:
        logger.exception(
            "assigned_ports_live failed router=%s org=%s",
            router.pk,
            org.pk,
        )
...
def _perform_client_isp_switch(
    router: MikroTikRouter,
    org,
    *,
    customer_id: int,
    ...
    uplink_mode = (router.uplink_mode or MikroTikRouter.UplinkMode.SINGLE).strip()
    if not _is_multi_isp_mode(uplink_mode):
        return {"ok": False, "error": "Client ISP switching requires multi-ISP setup."}

    customer = Customer.objects.filter(pk=customer_id, router=router).first()
    if not customer:
        return {"ok": False, "error": "Customer not found on this router."}

    member_ports = [
        str(p).strip() for p in (router.uplink_ports or []) if str(p).strip()
    ]
    if len(member_ports) < 2:
        return {"ok": False, "error": "Need at least two ISP ports configured."}
...
@client_workspace_required
@require_POST
def mikrotik_assigned_ports_apply(request, router_id: int):
    """Smart balance apply or manual client ISP switch from assigned-ports."""
    org = resolve_organization(request.user, request)
    if not org:
    ...
    if router.account_status == MikroTikRouter.AccountStatus.SUSPENDED:
        return JsonResponse(
            {"ok": False, "error": "Activate this MikroTik account first."},
            status=400,
        )

    action = (request.POST.get("action") or "").strip().lower()
    if action == "switch_client_isp":
        try:
            customer_id = int((request.POST.get("customer_id") or "").strip())
        except (TypeError, ValueError):
            return JsonResponse(
    ...
    api_host = _router_api_host(router)
    result = _schedule_assigned_smart_balance_apply(
        router,
        org,
        api_host,
        force=True,
...
def client_usage_analysis(request, customer_id: int):
    ...
    if can_live:
        ...
        if customer.service_type == Customer.ServiceType.HOTSPOT:
            ...
            has_user = bool((customer.pppoe_username or "").strip())
            error = (
                "No MikroTik router is available for this PPPoE client."
                if has_user
                else "Add a PPPoE username to collect live usage."
            )
...
def client_billing(request, customer_id: int):
    ...
    for pay in payments:
        # Same recovery path as /billing/dashboard/: prefer M-Pesa SMS receipt
        # (MpesaReceiptNumber) over CheckoutRequestID placeholders.
        pay.display_reference = heal_payment_mpesa_reference(pay)
        pay.display_phone = heal_payment_mpesa_phone(pay)
...
def client_usage_trends(request, customer_id: int):
    ...
    usage_filter = parse_usage_filter(request, default_time="1", default_range="day")
    hours = usage_filter["hours"]
    if (request.GET.get("export") or "").strip() in {"1", "true", "yes"}:
        from billing.usage_samples import client_usage_samples_export

        return JsonResponse(
            client_usage_samples_export(
                customer,
                hours=hours,
                since=usage_filter["since"],
                until=usage_filter["until"],
...
def clients_surfing_status(request):
    ...
    for customer in customers:
        ...
        hotspot_identities: set[str] = set()
        if service == "hotspot":
            try:
                from billing.devices import hotspot_macs_for_customer
                ...
                for mac in hotspot_macs_for_customer(customer):
                    compact = "".join(
                        ch for ch in (mac or "") if ch.isalnum()
                    ).upper()
                    if compact:
                        hotspot_identities.add(compact)
            except Exception:
                hotspot_identities = set()
            primary = "".join(
                ch for ch in (customer.hotspot_mac or "") if ch.isalnum()
            ).upper()
            if primary:
                hotspot_identities.add(primary)
            identity = primary or (next(iter(hotspot_identities), "") if hotspot_identities else "")

        def _any_identity_in(pool: set[str]) -> bool:
            if service == "hotspot":
                if not hotspot_identities:
                    return bool(identity and identity in pool)
                return any(mac in pool for mac in hotspot_identities)
            return bool(identity and identity in pool)

        def _first_identity_in(pool: set[str]) -> str:
            if service == "hotspot":
                for mac in hotspot_identities:
                    if mac in pool:
                        return mac
            return identity if identity in pool else identity
        ...
        entry: dict = {}
        if not identity and not hotspot_identities:
            ...
            session_online = _any_identity_in(active_any_router)
            connected = (
                _any_identity_in(connected_any_router) or session_online
                if service == "hotspot"
                else session_online
            )
            if not session_online:
            ...
            session_online = _any_identity_in(active_identities)
            connected = (
                _any_identity_in(connected_identities) or session_online
                if service == "hotspot"
                else session_online
            )
            # Hotspot clients can roam onto another org router — still count as connected.
            if service == "hotspot" and not connected:
                session_online = session_online or _any_identity_in(active_any_router)
                connected = _any_identity_in(connected_any_router) or session_online
            if not session_online:
        ...
        if service == "hotspot" and connected:
            match_mac = _first_identity_in(connected_any_router) or _first_identity_in(
                active_any_router
            ) or identity
            ...
            # Prefer live active session router when any linked MAC is active.
            active_router_id = _router_id_for_identity(
                match_mac,
                customer.router_id,
                active_by_router,
            )
            if active_router_id and _any_identity_in(
                active_by_router.get(active_router_id) or set()
            ):
...
def _hotspot_portal_context(org, *, mikrotik_login: bool = False, request=None):
    ...
    mpesa_callback = {"url": "", "ok": True, "warning": ""}
    if stk_ready:
        from core.hotspot_portal import mpesa_stk_callback_reachability

        mpesa_callback = mpesa_stk_callback_reachability(request, org)
        if mpesa_callback.get("warning") and not setup_hint:
            setup_hint = mpesa_callback["warning"]
    context = {
        "organization": org,
        "org_name": org.name,
        "page_title": title,
        ...
        "mpesa_callback_url": mpesa_callback.get("url") or "",
        "mpesa_callback_ok": bool(mpesa_callback.get("ok")),
        "mpesa_callback_warning": mpesa_callback.get("warning") or "",
        **access_ctx,
...
def _hotspot_welcome_url(
    ...
    *,
    ...
    """Build welcome URL, preserving MAC and signed payment session when known."""
    url = reverse("core:hotspot_welcome", kwargs={"join_code": join_code})
    params: dict[str, str] = {}
    mac_norm = _normalize_hotspot_mac(mac)
    if mac_norm:
        params["mac"] = mac_norm
    stk_value = (stk_id or "").strip()
    token_value = (token or "").strip()
    if stk_value.isdigit() and token_value:
        params["stk"] = stk_value
...
def _validate_hotspot_payment_session(payload, stk) -> str | None:
    """
    Fail closed when the signed payment token does not match this STK session.

    Tokens are always minted with the paying MAC; require it on every poll and
    activate so a leaked token cannot authorize a different device.
    """
    ...
    if paying_mac:
        if session_mac != paying_mac:
            return "Invalid payment session."
        return None
    if stk.customer_id and not customer_owns_hotspot_mac(stk.customer, session_mac):
        return "Invalid payment session."
    return None


def _hotspot_voucher_page_customer(org, request):
    """
    Resolve the customer whose unused voucher codes may be shown publicly.

    Requires a signed payment session or this device's MAC on an active package.
    """
    ...
    if stk_id.isdigit() and token:
        try:
            ...
            if payload.get("stk") == int(stk_id) and payload.get("org") == org.pk:
                ...
                if stk is not None and stk.customer is not None:
                    return stk.customer, ""
        except signing.BadSignature:
            pass
    ...
    for raw in (
        ...
        if hotspot_mac:
            break
    if not hotspot_mac:
        return None, "Open this page from your welcome screen after you connect."

    from billing.devices import customer_owns_hotspot_mac

    customer = _find_hotspot_customer_for_mac(org, hotspot_mac)
    if customer is None:
        return None, "No active package found for this device."
    if not customer_owns_hotspot_mac(customer, hotspot_mac):
        return None, "This device is not registered on your package."
    if not customer_can_surf_via_hotspot(customer):
        return None, "Connect this device first, then open vouchers from the welcome page."
    return customer, ""
...
def _hotspot_payment_start_impl(request, join_code: str):
    ...
    # Extra device joining a live unlimited package: authorize without charging again.
    # Voucher-capped plans never free-attach — they need a code or a new payment.
    from billing.devices import (
        authorized_hotspot_macs_for_customer,
        customer_devices_unlimited,
        customer_owns_hotspot_mac,
        ...
        return JsonResponse(
            {
                ...
                "pay_url": reverse(
                    "core:hotspot_pay", kwargs={"join_code": join_code}
                )
                + (f"?{urlencode({'mac': mac})}" if mac else ""),
            }
        )

    if (
        resolved.get("attached")
        and resolved.get("already_paid")
        and customer_devices_unlimited(customer)
    ):
        ...
        return JsonResponse(
            {
                ...
                "attached": True,
                "authorized": bool(
                    provision.get("ok") and provision.get("allowed") and mac_authorized
                ),
...
def hotspot_payment_status(request, join_code: str, stk_id: int):
    ...
    from accounts.security import AuthRateLimitExceeded, assert_public_stk_status_allowed
    from billing.models import StkPushRequest
    ...
    if org is None:
        return JsonResponse({"ok": False, "error": "Hotspot portal not found."}, status=404)
    try:
        assert_public_stk_status_allowed(request, stk_id, join_code=join_code)
    except AuthRateLimitExceeded as exc:
        return JsonResponse(
            {
                "ok": False,
    ...
    session_error = _validate_hotspot_payment_session(payload, stk)
    if session_error:
        return JsonResponse({"ok": False, "error": session_error}, status=403)
...
def hotspot_welcome(request, join_code: str):
    ...
    if stk_id.isdigit() and token:
        voucher_params["stk"] = stk_id
        voucher_params["token"] = token
    vouchers_url = reverse("core:hotspot_vouchers", kwargs={"join_code": join_code})
    if voucher_params:
        vouchers_url = f"{vouchers_url}?{urlencode(voucher_params)}"
...
def hotspot_vouchers(request, join_code: str):
    ...
    customer, access_error = _hotspot_voucher_page_customer(org, request)
    ...
    voucher_codes = (
        valid_hotspot_voucher_codes_for_customer(customer) if customer else []
    )
...
def hotspot_connection_status(request, join_code: str):
    ...
    want_live = (request.GET.get("live") or request.GET.get("session") or "").strip().lower() in {
        "1",
        "true",
        "yes",
    }
...
def hotspot_payment_activate(request, join_code: str, stk_id: int):
    ...
    # Prefer authorizing the paying device from this payment session.
    if session_mac or paying_mac:
        from billing.vouchers import activate_paid_subscription_stk

        activation = activate_paid_subscription_stk(
            stk,
            mac=session_mac or paying_mac,
            wait_first=True,
            quick=True,
        )
            ...
            return JsonResponse(
                {
                    ...
                    "queued": False,
                    "can_retry_authorize": not bool(activation.get("authorized")),
                    "offline": bool(activation.get("offline")),
                    "message": (
                        "Surfing allowed on MikroTik."
                        if activation.get("authorized")
                        else (
                            activation.get("authorization_error")
                            or "Waiting for MikroTik to allow surfing."
                        )
                    ),
                }
            )
